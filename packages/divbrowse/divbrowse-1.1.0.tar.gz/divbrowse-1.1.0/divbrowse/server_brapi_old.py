...
def create_app(filename_config_yaml = 'divbrowse.config.yml', config_runtime=None):
    ...
    @app.route("/brapi/v2/variantmatrix", methods = ['GET', 'POST', 'OPTIONS'])
    def __variantmatrix():

        if request.method == 'POST':
            input = process_request_vars(request.get_json(silent=True))
        elif request.method == 'GET':
            req_position_ranges = request.args.get('positionRanges', default = '', type = str)

            req_position_ranges_splitted = req_position_ranges.split(":")
            input = {}
            input['chrom'] = req_position_ranges_splitted[0]

            req_pos_splitted = req_position_ranges_splitted[1].split("-")
            input['startpos'] = int(req_pos_splitted[0])
            input['endpos'] = int(req_pos_splitted[1])
            #input['count'] = 100
            input['count'] = None

            _samples = request.args.get('germplasmDbIds', default = '', type = str)
            input['samples'] = _samples.split(",")
            input['variant_filter_settings'] = {}
        ...
        i = 0
        for sample in slice.samples_selected_mapped:
            _tmp = ["/".join(map(str, m)) for m in slice.sliced_variant_calls[i]]
            #calls.append(slice.sliced_variant_calls[i].tolist())
            calls.append(_tmp)
            i += 1

        brapi_result = {}
        brapi_result["callSetDbIds"] = input['samples']
        ...
        brapi_result["variants"] = []
        for _pos in slice.positions:
            brapi_result["variants"].append(        {
                "contig": str(input['chrom']),
                "end": int(_pos),
                "ploidy": 2,
                "start": int(_pos),
                "variantDbId": str(_pos)
            })
        ...
        if 'DP' in gd.available_calldata:
            ...
            for sample in  slice.samples_selected_mapped:
                DP_values[sample] = sliced_DP[i].tolist()
                brapi_result["fieldMatrix_DP"].append(DP_values[sample])
                i += 1
            result['calls_metadata']['dp'] = DP_values
        ...
        brapi_result_dict = {
            ...
            "result": {
                "callSetDbIds": brapi_result["callSetDbIds"],
                "data": calls,
                "expandHomozygotes": True,
                "genotypeFields": [
                    {
                        "fieldAbbreviation": "DP",
                        "fieldMatrix": brapi_result["fieldMatrix_DP"],
                ...
                "variants": brapi_result["variants"]
            }
        }



        return jsonify(brapi_result_dict)

...
def create_app(filename_config_yaml = 'divbrowse.config.yml', config_runtime=None):
    ...
    api = Api(app,
        version='1.0',
        title='DivBrowse API',
        description='OpenAPI specification of DivBrowse',
        default='divbrowse',
        default_label='divbrowse',
        ordered=True,
        base_url='/',
        doc='/apidoc',
        #doc = '/apidocs'
    ...
    #ns = api.namespace('divbrowse', description='DivBrowse API')
    ...
    def __pca():
        ...
        pca_result = calc_pca_for_slice_of_variant_calls(_result.snps_to_alt, _result.samples_selected_mapped)
    ...
    model_variant_call = api.model('VariantCall', {
        'sample_id': fields.List(fields.List(fields.Integer))
    })

    model_metadata_integer = api.model('VariantCallMetadataInteger', {
        'sample_id': fields.List(fields.Integer)
    })

    variant_summary_statistics = api.model('VariantSummaryStatistics', {
        'heterozygosity_freq': fields.List(fields.Float),
        'maf': fields.List(fields.Float),
        'missing_freq': fields.List(fields.Float),
        'vcf_qual': fields.List(fields.Float),
    })

    model_genetic_feature = api.model('GeneticFeature', {
        'ID': fields.String,
        'Ontology_term': fields.String,
        'Parent': fields.String,
        'description': fields.String,
        'end': fields.Integer,
        'phase': fields.Integer,
        'primary_confidence_class': fields.String,
    ...
    model_variants = api.model('Variants', {
        #'chromosomes': fields.List(fields.Nested(model_variant_call)),
        'coordinate_first': fields.Integer,
        'coordinate_first_chromosome': fields.String,
        'coordinate_first_next': fields.Integer,
        'coordinate_last': fields.Integer,
        'coordinate_last_chromosome': fields.String,
        'coordinate_last_prev': fields.Integer,
        #'samples': fields.List(fields.String)
        'variants_coordinates': fields.List(fields.Integer),
        'reference': fields.List(fields.String),
        'alternates': fields.List(fields.List(fields.String)),
        'dp': fields.Nested(model_metadata_integer),
        'dv': fields.Nested(model_metadata_integer),
        'variants_calls': fields.Nested(model_variant_call),
        'variants_summary_statistics': fields.Nested(variant_summary_statistics),
        'genetic_features': fields.List(fields.Nested(model_genetic_feature)),
        'nearest_genetic_features': fields.List(fields.Nested(model_genetic_feature)),
    })


    variants_input = api.model('VariantsRequestPayload', {
        'chrom': fields.String(description='Internal ID of the chromosome (as in the VCF)', example='chr1'),
        'startpos': fields.Integer(required=False, description='Start position for the variant calls slice', example=123456),
        'count': fields.Integer(description='Number of variants to be in the slice', example=50),
        'samples': fields.List(fields.String(description='A JSON array of the sample-IDs to be in the slice', example=['HOR_1111', 'HOR_1112']))
    })
    ...
    class Variants(Resource):
        ...
        @api.expect(variants_input)
        @api.response(200, 'Success', model_variants)
        def post(self):
            ...
            variants_summary_stats = gd.calc_variants_summary_stats(snps_to_alt)

            # impute with mean
            imputed = impute_with_mean(snps_to_alt)
            ref_vec = np.zeros(snps_to_alt.shape[1]).reshape(1, snps_to_alt.shape[1])
            log.debug("==== gd.get_slice_of_variant_calls() + gd.calc_variants_summary_stats() + impute_with_mean() => calculation time: %f", timer() - start)
    ...
    model_chromosome = api.model('Chromosome', {
        'id': fields.String,
        'label': fields.String,
        'start': fields.Integer,
        'end': fields.Integer,
        'centromere_position': fields.Integer,
        'number_of_variants': fields.Integer,
    })

    model_features = api.model('ConfigurationFeatures', {
        'blast': fields.Boolean,
        'pca': fields.Boolean
    })

    model_configuration = api.model('Configuration', {
        'chromosomes': fields.List(fields.Nested(model_chromosome)),
        'ploidy': fields.Integer,
        'count_variants': fields.Integer,
        'count_genotypes': fields.Integer,
        'count_elements': fields.Integer,
        'samples': fields.List(fields.String),
        'features': fields.Nested(model_features),
        'gff3': fields.Raw(),
        'dataset_descriptions': fields.Raw()
    })

    @api.route('/configuration')
    class Configuration(Resource):
        @api.marshal_with(model_configuration)
        def get(self):
            features = {

...
async def main(argv) -> int:
    ...
    itemsToSelect = [(item, item) for item in manifest.nodes.items()]
    
    selected_nodes = checkboxlist_dialog(
        title="Select Modules to Document",
        text="What of your models do you want to document?",
        values=itemsToSelect,
    ...
    #    message_dialog(
    #        title="Room service",
    #        text="You selected: %s\nGreat choice sir !" % ",".join(selected_nodes),
    #    ).run()
    #else:
    #    message_dialog("*Nothing Selected*").run()

...
class XTS(Broker):
    ...
    def order_event_handler(self,order):
        try:
            order['placed_by'] = order['LoginID']
            order['tag'] = order['LoginID']
            order['order_id'] = order['OrderReferenceID']
            order['exchange_order_id'] = order['ExchangeOrderID']
            order['exchange'] = order['ExchangeSegment']
            order['tradingsymbol'] = order['TradingSymbol']

            if order['exchange']=="NSEFO":
                order['exchange'] = "NFO"
            elif order['exchange']=="NSECM":
                order['exchange'] = "NSE"

            if order['exchange'] in ["NFO", "MCX"]:
                order["tradingsymbol"] = self.broker_symbol_map[order["tradingsymbol"]]

            order['order_type'] = order['OrderType'].upper()
            order['product']=order['ProductType'].upper()
            order['transaction_type']=order['OrderSide'].upper()
            order['quantity'] = int(order['OrderQuantity'])

            if 'OrderStopPrice' in order:
                order['trigger_price'] = float(order['OrderStopPrice'])
            else:
                order['trigger_price'] = None

            order['price'] = float(order['OrderPrice'])
            order['status'] = order["OrderStatus"].upper()

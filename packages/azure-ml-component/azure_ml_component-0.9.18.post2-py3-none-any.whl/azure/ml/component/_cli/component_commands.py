...
class CLIComponent(CLIOperations):
    ...
    def create(self, spec_file, package_zip, label, amlignore_file, fail_if_exists, version, manifest,
               params_override: List[str] = None, skip_if_no_change=False):
        """Create a component."""
        _verify_create_params(
...
def _create_component(workspace_name, resource_group_name, subscription_id, spec_file, package_zip, label,
                      ...
                      set: List[str] = None, skip_if_no_change=False):
    if logger is None:
        logger = _get_logger()
    ...
    return cli_component.create(spec_file=spec_file, package_zip=package_zip,
                                ...
                                manifest=manifest, params_override=set, skip_if_no_change=skip_if_no_change)
...
def _restore_component(
        workspace_name, resource_group_name, subscription_id, component_name, selector, registry=None, logger=None):
    if logger is None:
        logger = _get_logger()
    if registry and not workspace_name:
        workspace = None
...
def _component_update(workspace_name, resource_group_name, subscription_id, component_name, selector,
                      component_version, label, registry=None, logger=None, set: List[str] = None):
    if logger is None:
        logger = _get_logger()
    if registry and not workspace_name:
        workspace = None

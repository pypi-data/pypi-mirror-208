...
class VisualizationContext(object):
    ...
    @classmethod
    def from_pipeline_component(cls, pipeline_component: 'Pipeline',  # noqa: F821
    ...
    if node.data_set_definition:
        ...
        if def_val is not None:
            if def_val.data_set_reference is not None:
                ...
                source = next((s for s in data_sources if s.aml_data_store_name == literal_value.data_store_name
                               and s.relative_path == literal_value.relative_path), None)
            else:
                source = None

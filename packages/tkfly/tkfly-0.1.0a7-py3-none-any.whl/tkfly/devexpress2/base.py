...
class Widget(Frame):
    ...
    def configure(self, **kwargs):
        if "background2" in kwargs:
            color = kwargs.pop("background2")
            self._widget.FillColor = Color.FromArgb(color[0], color[1], color[2], color[3])
        if "foreground2" in kwargs:
            color = kwargs.pop("foreground2")
            self._widget.FillColor = Color.FromArgb(color[0], color[1], color[2], color[3])
...
if __name__ == '__main__':
    ...
    root = Tk()
    frame = Container(parent=root)
    Button(frame.frame()).pack(fill="both", expand="yes")

        ...
        for i, api in enumerate(items):
            ...
            for param in api.get("parameters", []):
                if param.get("in") == "query":
                    ...
                    param_string = f"|{param_name}|{'true' if param.get('required') else 'false'}" \
                                   f"|{param.get('type')}|{param_default}|{param_desc}|"
                    param_arr.append(param_string)
                elif param.get("in") == "header":
                    header_name = param.get('name')
                    header_desc = description_info.get(header_name) if description_info.get(header_name) else ""
                    param_string = f"|{header_name}|{'true' if param.get('required') else 'false'}" \
                                   f"|{param.get('type')}|{header_desc}|"
                    header_arr.append(param_string)
                elif param.get("in") == "body":
                    # body_string = f"|{param.get('name')}|{'true' if param.get('required') else 'false'}" \
                    #       f"|{param.get('schema',{}).get('type')}||"
                    # body_arr.append(body_string)
                    ...
                    if schema:
                        for property_name, property_info in schema.get("properties", {}).items():
                            ...
                            body_string = f"|{property_name}|{'true' if property_info.get('required') else 'false'}" \
                                          f"|{property_info.get('type')}|{property_default}|{property_desc}|"
                            body_arr.append(body_string)
            ...
            if response_dict:
                response_arr = [f'|{k}|{v}|' for k, v in response_dict.items()]
...
def swagger_json2markdown(data):
    route_str = data2md_str(data.get("paths"), "tag")
    title = ""
    info = data.get('info')
...
def swagger_convert(json_data, out_type, file_path=None):
    ...
    if out_type in ["yaml", "yml"]:
        ...
        result_data = swagger_json2markdown(json_data)
    else:

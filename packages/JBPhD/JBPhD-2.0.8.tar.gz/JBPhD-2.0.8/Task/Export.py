...
cpurun = subprocess.Popen([python_path, 'cpuexp.py'])
...
def upload():
    ...
    def create_folder(folder_path, parent_id):
        ...
        if len(results) > 0:
            ...
            #print(f'Found existing folder {folder_name} with ID {folder_id} in Google Drive')
            return folder_id
        else:
...
# Function to upload a file to Google Drive
    ...
    def upload_file(file_path, folder_id):
        ...
        if len(results) > 0:
            ...
            #print(f'Updated file {file_name} with ID {file.get("id")} in Google Drive')
        else:
            file_metadata = {'name': file_name, 'parents': [folder_id]}
            media = MediaFileUpload(file_path, resumable=True)
            file = drive_service.files().create(body=file_metadata, media_body=media, fields='id').execute()
            #print(f'Uploaded file {file_name} with ID {file.get("id")} to Google Drive')

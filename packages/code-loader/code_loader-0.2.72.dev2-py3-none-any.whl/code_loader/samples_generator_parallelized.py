...
class SamplesGeneratorParallelized:
    ...
    def _calculate_workers_by_hardware(self) -> int:
        p = psutil.Process(self.processes[0].pid)
        memory_usage_in_gb = p.memory_info().rss / 1024 / 1024 / 1024
        total_memory_in_gb = psutil.virtual_memory().total / 1024 / 1024 / 1024

        n_workers = min(int(multiprocessing.cpu_count() * 0.75),
                        int(total_memory_in_gb * 0.7 / memory_usage_in_gb))
    ...
    def start(self) -> None:
        ...
        if self.n_workers is None:
            n_workers = self._calculate_workers_by_hardware()

...
def datasets(run_id, train, ytrain, test, ytest, val, yval, hist=False, bins=50):
...
def results(run_id, y_predicted, y_true, threshold = 0.5, problem_type = 'binary classification'):
    ...
    pred_hist = plt.hist(y_predicted)
    
    freq = pred_hist[0]
    bins = pred_hist[1]

    data = {'run_id' : run_id, 'precision': prec, 'recall': rec, 'specificity': spec, 'f1': f1, 'accuracy': acc, 'npv': npv_val, 'c_matrix': c_matrix,
            'username': username, 'key': key, 'test_auc': roc_auc, 'freq': str(freq), 'bins': str(bins), 'cmatrix': str(c_matrix),
            'fpr': str(fpr), 'tpr': str(tpr)}

...
def results(run_id, y_predicted, y_true, threshold = 0.5, problem_type = 'binary classification', hist=False, bins=10):
    ...
    if hist==True:
      ...
      data = {'run_id' : run_id, 'precision': prec, 'recall': rec, 'specificity': spec, 'f1': f1, 'accuracy': acc, 'npv': npv_val, 'c_matrix': c_matrix,
              'username': username, 'key': key, 'test_auc': roc_auc, 'freq': str(freq), 'bins': str(bins),
              'fpr': str(fpr), 'tpr': str(tpr)}
    else:
       data = {'run_id' : run_id, 'precision': prec, 'recall': rec, 'specificity': spec, 'f1': f1, 'accuracy': acc, 'npv': npv_val, 'c_matrix': c_matrix,
              'username': username, 'key': key, 'test_auc': roc_auc, 'fpr': str(fpr), 'tpr': str(tpr)}     

...
class JSONDomainParser():
    ...
    def parse(self, json_dict):
        ...
        self.parser = JSONParser(ddd_name, self.ontology, domain_name)
            ...
            if is_iterator(query):
                for key, value in query.items():
                    if key in ["for_enumeration", "for_random_enumeration"]:
                        query_dict[key] = [self.parser.parse(item) for item in value]
                    else:
...
class JSONParser():
    ...
    def parse_plan_item(self, data):
        if plan_item.TYPE_FINDOUT in data:
            return self.parse_findout_plan_item(data[plan_item.TYPE_FINDOUT])
        if plan_item.TYPE_RAISE in data:
            return self.parse_raise_plan_item(data[plan_item.TYPE_RAISE])
    ...
    def parse_invoke_service_action_plan_item(self, data):
        ...
        return plan_item.InvokeServiceAction(self.ontology_name, service_action, preconfirm, postconfirm, downdate_plan)

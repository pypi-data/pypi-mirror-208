...
class DDDComponentManager(object):
    ...
    def get_ontology(self, name, domain_orchestration):
        if name not in self.ontologies:
            self.load_ddd_for_ontology_name(name, domain_orchestration)

        return self.ontologies[name]

    def load_ddd_for_ontology_name(self, name, ddds_as_json):
    ...
    def get_ddd_specific_components_of_ontology_name(self, name, domain_orchestration):
        if name not in self.ontologies:
            self.load_ddd_for_ontology_name(name, domain_orchestration)
        ontology = self.get_ontology(name)

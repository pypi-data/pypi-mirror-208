...
class Commitments(AsSemanticExpressionMixin, AsJSONMixin):
    def __init__(self, content=[], previous_commitment_lifespan=30, meta_com={}, pcom=[], meta_pcom={}):
        self._backing_set = Set(content)
        self._meta_com = meta_com
        self._pcom = Set(pcom)
        self._meta_pcom = meta_pcom
    ...
    def create_from_json(cls, input):
        ...
        meta_pcom = {parser.parse(elem["key"]): (elem["value"][0], int(elem["value"][1])) for elem in input["meta_pcom"]}
        return cls(commitments, lifespan, meta_com, pcom, meta_pcom)
    ...
    def as_dict(self):
        return {
            ...
            "meta_com": [{"key": k, "value": [v[0], v[1]]} for k, v in self._meta_com.items()],
            "pcom": self._pcom,
            "meta_pcom": [{"key": k, "value": [v[0], v[1]]} for k, v in self._meta_com.items()],
    ...
    def add(self, proposition, turn_number=-1, topmost_goal=None):
        ...
        self.meta_com[proposition] = (topmost_goal, turn_number)
    ...
    def remove(self, proposition):
        if proposition in self.meta_com:
            self.meta_pcom[proposition] = self.meta_com[proposition]
    ...
    def should_purge_previous_commitments(self, current_turn_number):
        for fact in self.pcom:
            if fact in self.meta_pcom:
                (_, turn_number) = self.meta_pcom[fact]

...
lint:  ## run static analysis with flake8
	python -m black --check nikefy setup.py
	python -m flake8 nikefy setup.py

# Alias
lints: lint

format:  ## run autoformatting with black
	python -m black nikefy/ setup.py
...
annotate:  ## run type checking
	python -m mypy ./nikefy

#########
# TESTS #
#########
test: ## clean and run unit tests
	python -m pytest -v nikefy/tests

coverage:  ## clean and run unit tests with coverage
	python -m pytest -v nikefy/tests --cov=nikefy --cov-branch --cov-fail-under=75 --cov-report term-missing

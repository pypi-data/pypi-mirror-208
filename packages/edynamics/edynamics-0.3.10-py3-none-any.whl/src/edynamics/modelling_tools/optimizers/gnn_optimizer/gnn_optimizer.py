...
def gnn_optimizer(embedding: Embedding,
    ...
    pbar_0 = tqdm(range(len(observers)), leave=True)
    # todo: run first iteration as base line
    for i in pbar_0:
        pbar_0.set_description('Iteration ' + str(i))
        ...
        if compute_pool is not None:
            ...
            pbar_1 = tqdm(observers, leave=False)
            for _, observer_ in enumerate(pbar_1):
                pbar_1.set_description(observer_.observation_name)
                # add a new observer
                embedding_copy = deepcopy(embedding)
                embedding_copy.observers[-1] = observer_
                embedding_copy.compile()
        ...
        if verbose:
            print('\n' + maximum[0].observation_name + ' ' + str(improvement))

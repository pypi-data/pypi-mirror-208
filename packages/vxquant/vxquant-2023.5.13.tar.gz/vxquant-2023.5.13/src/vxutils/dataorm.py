...
SHARED_MEMORY_DATABASE = "sqlite:///vxquantdb?mode=memory&cache=shared"
...
class vxDataBase:
    ...
    def __init__(self, db_uri: str = None, **kwargs) -> None:
        db_uri = db_uri or SHARED_MEMORY_DATABASE
...
if __name__ == "__main__":
    ...
    class vxTest(vxDataClass):
        ...
        name = vxField("")
        age = vxIntField(0, 1)
        score = vxFloatField(0, 2, -100)
        status = vxEnumField(Status.SUCCESS)

    db = vxDataBase("sqlite:///./data/test1.db")
    db.create_table("st", ["id"], vxTest, if_exists="replace")
    db.create_table("test2", ["position_id"], vxCashPosition, if_exists="replace")
    p = vxCashPosition()
    with db.start_session() as session, vxtime.timeit():
        session.save("test2", p)
        objs = [
            vxTest(id=f"uuid_{i}", name=f"name_{i}", age=18 + i, score=100 + i)

...
import sqlite3
import contextlib
from typing import Iterator, List, Optional, Type, Union, Dict
...
class vxDBORM:
    ...
    def __init__(self, dbpool: Union[Dict, vxPooledDB] = None, *args, **kwargs) -> None:
        if not dbpool:
            dbpool = vxPooledDB(creator=sqlite3, database=SHARED_MEMORY_DATABASE)
        elif isinstance(dbpool, dict):
            dbpool = vxPooledDB(*args, **kwargs)
        elif not isinstance(dbpool, vxPooledDB):
            raise ValueError("dbpool must be vxPooledDB or dict")
    ...
    def connection(self):
        """数据库连接"""
        return self._dbpool.connection()

    @contextlib.contextmanager
    def start_session(self):
        """开始session，锁定python线程加锁，保障一致性"""
        with self.connection(), self._lock:
            yield self
    ...
    def distinct(self, table_name: str, col: str, *conditions, **query) -> List:
        ...
        return [item[col] for item in self.connection().execute(sql)]

    def save(self, table_name: str, *vxdataobjs: Any) -> "vxDBORM":
        """保存数据对象到数据库

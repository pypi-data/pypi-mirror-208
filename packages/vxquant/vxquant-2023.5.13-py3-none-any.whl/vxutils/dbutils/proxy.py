"""DBProxy - hardened DB-API 2 connections proxy.
...
class DBProxyError(Exception):
    """General DBProxyError error."""


class InvalidCursor(DBProxyError):
...
class DBConnectionProxy:
    ...
    def executescripts(self, sql_scripts, *args, **kwargs):
        """Execute a SQL statement."""
        with self:
            return self.cursor().executescripts(sql_scripts, *args, **kwargs)
    ...
    def cursor(self, *args, **kwargs):
        ...
        return DBCursorProxy(self, *args, **kwargs)
...
class DBCursorProxy:
    ...
    def __init__(self, con, *args, **kwargs):
        ...
        try:
            ...
            raise TypeError(f"{con!r} is not a DBConnectionProxy.")
        self._closed = False
...
if __name__ == "__main__":
    ...
    with vxtime.timeit():
        db = DBConnectionProxy(creator=sqlite3, database="data/cache.db")
        ...
        for i in range(100):
            db.execute("INSERT INTO test (name) VALUES (?)", (f"test{i}",))
        cur = db.cursor()

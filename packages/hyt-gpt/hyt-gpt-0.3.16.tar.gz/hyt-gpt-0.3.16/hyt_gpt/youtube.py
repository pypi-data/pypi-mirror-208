...
class HytGpt:
    ...
    def fetch_subtitles(
        ...
        if not subtitles:
            try:
                subtitles, language = self.transcribe_youtube_video(
                    ylink, task_instance
                )
    ...
    def summarize(
        ...
        # HytGpt.update_progress(task_instance, 0.2, 0.9, 10)
        summaried_text = ""
        i = 1

        print(f">>>>> Task instance outer: {task_instance}")

        progress_queue = Queue()
        # Start updating progress in the background
        progress_thread = threading.Thread(target=HytGpt.update_progress, args=(task_instance, progress_queue, 0.2, 0.9, 10))
        ...
        # Wait for the progress update thread to finish
    ...
    def transcribe_youtube_video(
        ...
        for i, chunk in enumerate(audio_chunks):
            ...
            HytGpt.update_task_instance(
                task_instance, 0.2 + 0.7 * (i + 1) / len(audio_chunks)
            )
            os.remove(chunk)
    ...
    @staticmethod
    def update_progress(task_instance, progress_queue, start, end, duration):
        print("Starting progress update thread")
        increment = (end - start) / duration
        progress = start
        print(f">>>>> Task instance inner: {task_instance}")

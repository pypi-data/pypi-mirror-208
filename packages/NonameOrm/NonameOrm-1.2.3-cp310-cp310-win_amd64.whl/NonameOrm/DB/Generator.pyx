...
cdef class IsExistsGenerator(SqlGenerator):
    def __init__(self, model, FilterListCell query):
        self.From(model)
        self.whereCol = query

    cdef void set_join(self, list join_list):
        self._joinList = join_list

    cpdef public tuple Build(self):
        cdef:
            str sql = "SELECT 1 FROM " + self.target + " "
            str where_sql
            tuple temp
            list params

        if self.joinList.__len__():
            sql = sql + self.build_join() + " "

        where_sql, params = SqlGenerator.build_where(self.whereCol)
        return sql + where_sql, tuple(params)

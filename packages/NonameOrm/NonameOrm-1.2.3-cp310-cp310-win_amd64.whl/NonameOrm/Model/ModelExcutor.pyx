...
from NonameOrm.DB.Generator cimport SqlGenerator, sqlType, IsExistsGenerator
...
cdef class BaseModelExecutor:
    ...
    cpdef public object _is_exists(self, FilterListCell query):
        cdef:
            IsExistsGenerator generator = IsExistsGenerator(self.model, query)
            str sql
            tuple args

        # 如果当前执行器的sql 含有join 列表，则传入到新的 sql 对象
        if self.sql.joinList.__len__():
            generator.set_join(self.sql.joinList)

        sql, args = generator.Build()

        return DB.getInstance().executeSql(sql + " limit 1", args=args)


    cpdef public int count(self, FilterListCell query):
        cdef:
            str sql
            tuple args
            int count

        self.sql.where(query)

        sql, args = SqlGenerator.build_where(self.sql.whereCol)
        cdef str join_temp = self.sql.build_join() if len(self.executor.sql.joinList) else ""

        count = self.db.executeSql(f"select count({self.model.tableName}.{self.model.pkName}) from {self.model.tableName} {join_temp} {sql}", args=tuple(args))
        return count

    cpdef bint is_exists(self, FilterListCell query):
        res = self._is_exists(query)

        return bool(res.__len__())
...
cdef class AsyncModelExecutor(BaseModelExecutor):
    ...
    async def is_exists(self, FilterListCell query):
        res = await self._is_exists(query)
        return bool(res.__len__())
    ...
    async def findForeignKey(self, ModelInstance instance, bint deep=False, dict path = None, int clevel=1, int level=3, str before=None):
        ...
        for fk in self.model.fk:
            if not fk.auto_fetched: continue
            # 循环查找
            ...
            if fk.Type == ForeignType.MANY_TO_MANY:
                ...
                if fk.target.tableName in path and instance[fk.bindCol.name] in path[fk.target.tableName] and fk.target is not self.model and False:
                    instance[fk.name] = path[fk.target.tableName][instance[fk.bindCol.name]]
                    continue
                else:
    ...
    async def _saveOTM(self, fk, instance):
        exc = fk.target.getExecutor(self.work)
        print(fk.bindCol.name, fk.target.tableName, instance)
        for model in instance[fk.name]:
            exc.reset()
            model[fk.targetBindCol.name] = instance[fk.bindCol.name]
    ...
    async def update(self, instance):
        ...
        cdef:
            BaseProperty cur
            list params = [cur.updateCell(instance[cur.name]) for cur in self.model.col if cur.name in instance and instance]

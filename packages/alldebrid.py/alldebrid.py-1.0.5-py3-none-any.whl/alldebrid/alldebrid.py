...
class StreamLinkProcessor:
    ...
    def _try_get_delayed_link(self, link: str, downloader, max_attempts, retry_delay, max_delay) -> Optional[str]:
        ...
        stream_id = unlock_response.get("data", {}).get(
            "streams", [{}])[0].get("id")
        ...
        while attempts < max_attempts:
            delayed_link_response = downloader.delayed_links(
            ...
            if status == 2:
                ...
                if elapsed_time >= time_limit:
                    raise TimeoutError(
                        "Max delay reached. Cannot get direct link.")
            time.sleep(retry_delay)
            attempts += 1

        raise MaxAttemptsExceededException(
            "Max attempts reached. Cannot get direct link.")
...
class AllDebrid:
    ...
    def download_file_then_upload_to_alldebrid(self, urls: Union[str, List[str]]) -> Union[Dict, List[Dict]]:
        """
        Download a file or multiple files from given urls and upload them to AllDebrid.

        Parameters
        ----------
        urls : Union[str, List[str]]
            The url or list of urls to download the files from.

        Returns
        -------
        Union[Dict, List[Dict]]
            Response of the API or a list of responses from the API.
        """
        if isinstance(urls, str):
            # Single URL
            return self._download_and_upload_single_file(urls)
        elif isinstance(urls, list):
            # List of URLs
            with concurrent.futures.ThreadPoolExecutor() as executor:
                futures = [executor.submit(
                    self._download_and_upload_single_file, url) for url in urls]
            ...
            raise TypeError(
                "Invalid argument type, expected str or list of str")

    def _download_and_upload_single_file(self, url: str) -> Dict:
        """
        Download a single file from a given url and upload it to AllDebrid.
    ...
    def check_pin(self, pin_response=None, hash_value=None, pin=None) -> dict:
        ...
        if pin_response is None and (hash_value is None or pin is None):
            raise ValueError(
                "Either pin_response or hash and pin must be provided")
    ...
    def upload_file(self, file_paths: List[str]) -> dict:
        ...
        for i, file_path in enumerate(file_paths):
            with open(file_path, 'rb') as file:
                files[f"files[{i}]"] = (
                    file.name, file.read(), 'application/x-bittorrent')
    ...
    def get_magnet_status(self, magnet_id: int) -> dict:
        ...
        response = self._request(
            method="GET", endpoint=endpoint, params={"id": magnet_id})
    ...
    def restart_magnet(self, magnet_id: Optional[int] = None, ids: Optional[List[int]] = None) -> dict:
        ...
        if magnet_id is not None and ids is not None:
            raise ValueError(
                "Only one of magnet_id or ids can be provided for restart magnet")
    ...
    def check_magnet_instant(self, magnets: Union[str, List[str]] = None) -> dict:
        ...
        response = self._request(
            method="POST", endpoint=endpoint, magnets=magnets)
    ...
    def purge_recent_links(self) -> dict:
        ...
        if not endpoint:
            raise ValueError(
                "Endpoint URL not found for purging recent links.")
    ...
    def _get_session(self):
        if self.session is None:
            ...
            self.session.mount(
                'http://', requests.adapters.HTTPAdapter(pool_connections=10, pool_maxsize=50))
            self.session.mount(
                'https://', requests.adapters.HTTPAdapter(pool_connections=10, pool_maxsize=50))

...
def train_loop(
  ...
  Args:
    ...
    picsellia_experiment: Decide whether or not you want to log loss at each step to Picsellia.
      ...
      with tf.compat.v2.summary.record_if(
        ...
        for _ in range(global_step.value(), train_steps,
          ...
          if log_metrics is not None:
            log_metrics(experiment=picsellia_experiment, tf_metrics_dir=picsellia_experiment.checkpoint_dir, global_step=global_step.value(), metrics_type='train')
          ...
          if ((int(global_step.value()) - checkpointed_step) >=
            ...
            if evaluate_fn is not None and log_metrics is not None:
                evaluate_fn(picsellia_experiment.metrics_dir, 
                            picsellia_experiment.config_dir, 
                            picsellia_experiment.checkpoint_dir,
                            train_steps=1
                            )
                
                log_metrics(experiment=picsellia_experiment, tf_metrics_dir=picsellia_experiment.metrics_dir, global_step=global_step.value(), metrics_type='eval')

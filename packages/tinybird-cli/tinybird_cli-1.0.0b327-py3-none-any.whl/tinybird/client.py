...
class TinyB(object):
    ...
    async def create_token(self, name: str, scope: str, origin_code: Optional[str], origin_resource_name_or_id: Optional[str]):
        ...
        if len(url + '?' + query_string) > MAX_GET_LENGTH:
            return await self._req(url, method='POST', data=data)
        else:
            url = url + '?' + query_string
            return await self._req(url, method='POST', data='')
    ...
    async def alter_tokens(self, name: str, scopes: List[str]):
        ...
        query_string = urlencode(data)
        if len(url + '?' + query_string) > MAX_GET_LENGTH:
            return await self._req(url, method='PUT', data=data)
        else:
            url = url + '?' + query_string
            return await self._req(url, method='PUT')

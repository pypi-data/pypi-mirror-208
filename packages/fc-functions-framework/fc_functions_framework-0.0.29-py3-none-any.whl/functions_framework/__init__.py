...
_DAPR_PUBSUB = "pubsub"
global GPRC_HOST 
_DAPR_SIDECAR_MODE = "sidecar"
_DAPR_STANDALONE_MODE = "standalone"
    ...
    if inputs is not None:
        for input in inputs.values():
            ...
            if type == _DAPR_BINDINGS:
                ...
                # 判断模式是sidecar还是standalone
                grpc_host = os.getenv("DAPR_HOST","")
                if grpc_host == "":
                    pass
                else :
                    key = _app._servicer._topic_map.keys()
                    _app._servicer._topic_map["{}:{}:{}".format(input.componentName,input.uri,input.uri)]=_app._servicer._topic_map.pop(list(key)[0])

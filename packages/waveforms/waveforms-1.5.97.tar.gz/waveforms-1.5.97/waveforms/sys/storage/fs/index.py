...
from .chunk import (CHUNKSIZE, delete_chunk, load_chunk, load_pack_chunk,
                    pack_chunk, pack_reader, pack_writer, save_chunk)
...
class FileChunk(Base):
    ...
    pack = Column(Integer, default=None)
    ...
    def chunk(self):
        ...
        if self.pack is not None:
            with pack_reader(self.pack) as reader:
                return load_pack_chunk(reader, self.offset, self.pack_size)
        else:
...
def before_insert_file_chunk(mapper, connection, target):
    target.chunk_hash = save_chunk(target._chunk)
...
def pack_chunks(db: Session):
    for chunk in db.query(FileChunk).filter(FileChunk.pack == None).order_by(
            FileChunk.id).limit(100):
        writer, name = pack_writer()
        with writer:
            offset, size = pack_chunk(writer, chunk.chunk)
            chunk.pack = name
            chunk.offset = offset
            chunk.pack_size = size
            chunk.compressed = True
        try:
            db.commit()
            delete_chunk(chunk.chunk_hash)
        except:
            db.rollback()

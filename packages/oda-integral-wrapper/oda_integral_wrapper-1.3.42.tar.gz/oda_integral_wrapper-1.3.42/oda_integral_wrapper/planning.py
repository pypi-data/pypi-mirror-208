...
def get_current_pad(rev_num=0, num_ao=2, columns_to_remove=['KEY_PROP_ID']):
    ...
    for ao_i in range(ao_start, ao_stop+1):
        ...
        current_hostname = socket.gethostname()
        if 'lesta' in current_hostname:
            arc_host_url = arc_lesta_url
        else:
            arc_host_url = arc_url
        pad_pattern = arc_host_url + "/aux/org/AO%02d/pad_%02d" % (ao_i, ao_i) + "_%04d.fits"
        pad_list = []
...
def get_pointings(revid, kind="historic", getdata=True, max_version=5):
    ...
    current_hostname = socket.gethostname()
    if 'lesta' in current_hostname:
        historic_host_url = cons_lesta_url
        predicted_host_url = pvphase_lesta_url
    else:
        historic_host_url = cons_url
        predicted_host_url = pvphase_url

    if kind == 'historic':
        pt = historic_host_url + "/aux/adp/%s.001/pointing_definition_%s.fits.gz" % (revid, kind)
    elif kind == 'predicted':
        pt = predicted_host_url + "/aux/adp/%s.000/pointing_definition_%s_" % (revid, kind)
    else:
...
def get_pod(revid):
    ...
    for num in range(1, 10):
        current_hostname = socket.gethostname()
        if 'lesta' in current_hostname:
            nrt_host_url = pvphase_lesta_url
        else:
            nrt_host_url = pvphase_url
        pod_test = nrt_host_url + "/aux/org/%s/pod_%s_%04d.fits" % (revid, revid, num)
        try:
            ...
            current_hostname = socket.gethostname()
            if 'lesta' in current_hostname:
                cons_host_url = cons_lesta_url
            else:
                cons_host_url = cons_url
            pod_test = cons_host_url + "/aux/org/%s/pod_%s_%04d.fits.gz" % (revid, revid, num)
            try:
                ff = fits.open(pod_test)
                pod_list.append(pod_test)
                ff.close()

...
class KrxKindWeb:
    ...
    def get_document_link(self, doc_id):
        ...
        if self.file_cache_dir is not None:
            cache_base_dir = os.path.join(self.file_cache_dir, "krx_links", yyyy, mm)
        ...
        if text is None:
            ...
            if self.file_cache_dir is not None:
                cache_base_dir = os.path.join(self.file_cache_dir, "links", yyyy, mm)
    ...
    def get_document_html(self, doc_id) -> Tuple[str, str]:
        ...
        yyyy = doc_id[0:4]
        mm = doc_id[4:6]
        cache_file_name = "html_{}.html"
        if self.file_cache_dir is not None:
            cache_base_dir = os.path.join(self.file_cache_dir, "krx_docs", yyyy, mm)
            file_path = os.path.join(cache_base_dir, cache_file_name.format(doc_id))
            if os.path.exists(file_path):
                logger.info(f"file exists, use cache : {file_path}")

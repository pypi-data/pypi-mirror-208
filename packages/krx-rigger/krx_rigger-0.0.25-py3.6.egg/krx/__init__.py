...
class KrxKindWeb:
    def __init__(self, cache: AdtCache = None, file_cache_dir: str = None):
        ...
        if cache is None:
            logger.info("Cache is not provided. Use MemoryCache")
            cache = MemoryCache()

        self.cache = cache
        self.file_cache_dir = file_cache_dir
        if file_cache_dir is not None:
            logger.info(f"File cache is enabled. Cache directory: {file_cache_dir}")
    ...
    def get_document_link(self, doc_id):
        text = self._read_cache(cache_file_name="link_{}.html", dir_prefix="krx_links", doc_id=doc_id)
        ...
        if text is None:
            ...
            text = response.text
            self._write_cache(cache_file_name="link_{}.html", dir_prefix="krx_links", doc_id=doc_id, text=text)
    ...
    def _read_cache(self, cache_file_name, dir_prefix, doc_id):
        ...
        if self.file_cache_dir is not None:
            ...
            if os.path.exists(file_path):
                logger.info(f"{doc_id} file exists, use cache : {file_path}")
    ...
    def get_document_html(self, doc_id, link=None) -> Tuple[str, str]:
        ...
        if html_link is None:
            ...
            logger.info(f"doc_id : {doc_id}, link : {html_link}")

        text = self._read_cache(cache_file_name="html_{}.html", dir_prefix="krx_docs", doc_id=doc_id)
        if text is not None:
            return text, link
        ...
        if response.status_code != 200:
            logger.error(f"doc_id : {doc_id}, link : {html_link}, status_code : {response.status_code}")
            return None, html_link

        response.encoding = 'utf-8'
        text = response.text
        self._write_cache(cache_file_name="html_{}.html", dir_prefix="krx_docs", doc_id=doc_id, text=text)

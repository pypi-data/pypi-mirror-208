    ...
    def _instrument(self, **kwargs):
        ...
        def server(*args, **kwargs):
            if "interceptors" in kwargs:
                ...
                if type(interceptors) == tuple:
                    interceptors = list(interceptors)
                kwargs["interceptors"] = [server_interceptor(tracer_provider=tracer_provider, filter_=self._filter)] + interceptors
            else:
                kwargs["interceptors"] = [
                    server_interceptor(
    ...
    def _instrument(self, **kwargs):
        ...
        def server(*args, **kwargs):
            if "interceptors" in kwargs:
                ...
                if type(interceptors) == tuple:
                    interceptors = list(interceptors)
                kwargs["interceptors"] = [aio_server_interceptor(tracer_provider=tracer_provider, filter_=self._filter)] + interceptors
            else:
                kwargs["interceptors"] = [
...
class GrpcAioInstrumentorClient(BaseInstrumentor):
    ...
    def _add_interceptors(self, tracer_provider, kwargs):
        ...
        if "interceptors" in kwargs and kwargs["interceptors"]:
            current_interceptors = kwargs["interceptors"]
            if type(current_interceptors) == tuple:
                current_interceptors = list(current_interceptors)
            kwargs["interceptors"] = interceptors + current_interceptors
        else:

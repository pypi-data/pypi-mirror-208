...
"""   
@Author    : Gavin
@DateTime  : 2021/5/26 15:35
@Contact   : wen.guo@foxmail.com

@Project   : AutoTestTool
@File      : scaffold.py
@Describe  : 测试用例脚手架 - 根据 .har 文件生成对应测试用例
————————————————————
@Version   : 0.1 
"""
# update   : 05-26
...
def create_scaffold(project_name):
    ...
    def show_tree(prj_name):
        try:
            # print(f"\n$ tree {prj_name} -a")
            print(f"\ntree {prj_name} /f")
            # subprocess.run(["tree", prj_name, "-a"])    # mac
            subprocess.run(["tree", prj_name, "/f"])    # win
            print("")
...
def main_scaffold(args):
    logger.info("startproject with scaffold")

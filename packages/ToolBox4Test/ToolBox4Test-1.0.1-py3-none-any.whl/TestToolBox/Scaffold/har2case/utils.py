...
import yaml
from Scaffold.har2case.compat import bytes, ensure_ascii, str, unquote
...
def convert_x_www_form_urlencoded_to_dict(post_data):
    ...
    if isinstance(post_data, str):
        ...
        for k_v in post_data.split("&"):
            ...
            converted_dict[key] = json.loads(unquote(value))
            # print(json.loads(unquote(value)))
...
def dump_yaml(testcase, yaml_file):
    ...
    with io.open(yaml_file, 'w', encoding="utf-8") as outfile:
        ...
        yaml.dump(testcase, outfile, allow_unicode=True, default_flow_style=False, indent=2, sort_keys=False)

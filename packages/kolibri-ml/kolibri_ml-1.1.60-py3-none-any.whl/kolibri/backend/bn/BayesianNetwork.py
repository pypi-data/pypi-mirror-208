...
from kolibri.backend.bn.ContinuousFactor import ContinuousFactor
from kolibri.backend.bn.DiscreteFactor import (
    DiscreteFactor)
from kolibri.backend.bn.JointProbabilityDistribution import JointProbabilityDistribution
...
class BayesianNetwork(DAG):
    ...
    def fit(
        ...
        **kwargs,
        ...
        """
        from kolibri.backend.bn.MLE import MaximumLikelihoodEstimator
        from kolibri.backend.bn.base_estimator import BaseEstimator
    ...
    def simulate(
        ...
        """
        from kolibri.backend.bn.Sampling import BayesianModelSampling
    ...
    def save(self, filename, filetype="bif"):
        ...
        if filetype == "bif":
            from kolibri.backend.bn.readwrite import BIFWriter

            writer = BIFWriter(self)
            writer.write_bif(filename=filename)

        elif filetype == "uai":
            from kolibri.backend.bn.readwrite import UAIWriter

            writer = UAIWriter(self)
            writer.write_uai(filename=filename)

        elif filetype == "xmlbif":
            from kolibri.backend.bn.readwrite import XMLBIFWriter
    ...
    def load(filename, filetype="bif"):
        ...
        if filetype == "bif":
            from kolibri.backend.bn.readwrite import BIFReader

            reader = BIFReader(path=filename)
            return reader.get_model()

        elif filetype == "uai":
            from kolibri.backend.bn.readwrite import UAIReader

            reader = UAIReader(path=filename)
            return reader.get_model()

        elif filetype == "xmlbif":
            from kolibri.backend.bn.readwrite import XMLBIFReader

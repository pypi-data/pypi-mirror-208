...
class GDSPainter(Painter):
    def __init__(self, name, out_file, rotation=0):
        super(GDSPainter, self).__init__(out_file, rotation=0)
        self.name = str(name)
        self.root = gdspy.Cell("MAIN")

        cell = gdspy.Cell(name)
        self.root.add(gdspy.CellReference(cell, rotation=rotation))
        self.layout = GDSLayout(cell)

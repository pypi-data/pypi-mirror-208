...
class DependentComponentExecutor(ExecutorBase):
    ...
    def _refine_entity_args(cls, entity_args: dict, return_annotation: dict, force_inject_deps: bool) -> dict:
        ...
        has_control_output = force_inject_deps or bool(
            any([getattr(o, "is_control", False) for o in return_annotation.values()])
        )
        core_env = entity_args.get("environment", cls._get_default_env(has_control_output))
        if isinstance(core_env, Environment):
            core_env = core_env._to_dict()
        core_env = cls._update_environment(core_env, force_inject_deps, has_control_output)

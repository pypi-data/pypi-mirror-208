...
namespace DL0
{
    ...
    {
        // no num_modules in DL0 model! Nothing here!
    }
    ...
    //   GET PIXEL STATUS                                                //
    ///////////////////////////////////////////////////////////////////////
    const uint16* Telescope::Event::getPixelStatus() const
    {
        if (_alloc_proto_object==false)
            throw std::runtime_error("memory not allocated for protobuf object");
        assert(proto_object->num_pixels_survived() != 0);
        return ADH::AnyArrayHelper::readAs<uint16>(proto_object->pixel_status());
    }

    uint16* Telescope::Event::getPixelStatus()
    {
        if (_alloc_proto_object==false)
            throw std::runtime_error("memory not allocated for protobuf object");
        assert(proto_object->num_pixels_survived() != 0);
    ...
    //   GET FIRST CELL ID                                               //
    ///////////////////////////////////////////////////////////////////////
    const uint32* Telescope::Event::getFirstCellId() const
    {
        if (_alloc_proto_object==false)
            throw std::runtime_error("memory not allocated for protobuf object");
        assert(proto_object->num_pixels_survived() != 0);
        return ADH::AnyArrayHelper::readAs<uint32>(proto_object->first_cell_id());
    }

    uint32* Telescope::Event::getFirstCellId()
    {
        if (_alloc_proto_object==false)
            throw std::runtime_error("memory not allocated for protobuf object");
        assert(proto_object->num_pixels_survived() != 0);
        return ADH::AnyArrayHelper::reallocAs<uint32>(proto_object->mutable_first_cell_id(),
                                                      proto_object->num_pixels_survived());
    }

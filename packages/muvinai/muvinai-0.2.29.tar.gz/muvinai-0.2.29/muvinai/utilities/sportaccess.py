...
great
    ...
    fv = datetime.strptime(proclub['Vigencia'], '%d/%m/%Y').replace(tzinfo=arg_tz)
    ...
    socio = {"_id": None,
             "status": status,
             "fecha_vigencia": fv,
             "last_subscription_date": datetime.strptime(proclub["FechaIngreso"], '%d/%m/%Y').replace(tzinfo=arg_tz),
             "apto_medico": {
                 "fecha_vigencia": datetime.strptime(proclub['AptoMedico'], '%d/%m/%Y').replace(tzinfo=arg_tz)
             },

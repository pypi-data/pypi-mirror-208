...
from commitguard.utils import (
    GitError,
    exec_git,
    get_git_directory_path,
    get_git_hook_directory_path,
    get_project_commitguard_plugins_path,
...
class ExecGitTestCase(unittest.TestCase):
    def test_exec_fail(self):
        with tempdir(change_into=True), self.assertRaisesRegex(
            GitError,
            r"Git command '\['git', 'foo'\]' returned non-zero exit " "status 1",
        ) as err:
            exec_git("foo")
...
class GetProjectCommitguardPluginsPathTestCase(unittest.TestCase):
    def setUp(self):
        self.tempdir = TemporaryDirectory()
        self.temp_path = Path(self.tempdir.name).resolve()
    ...
    def test_with_subpath(self):
        ...
        commitguard_plugins_path = get_project_commitguard_plugins_path(sub_path)
        self.assertEqual(commitguard_plugins_path, self.temp_path / ".commitguard")

    def test_with_env_pwd(self):
        os.chdir(str(self.temp_path))

        commitguard_plugins_path = get_project_commitguard_plugins_path()
        self.assertEqual(commitguard_plugins_path, self.temp_path / ".commitguard")

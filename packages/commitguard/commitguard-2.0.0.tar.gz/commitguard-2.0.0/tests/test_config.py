...
class CommitguardConfigTestCase(unittest.TestCase):
    def test_load_from_toml_file(self):
        ...
        self.assertListEqual(config.get_pre_commit_script_names(), ["foo", "bar"])
    ...
    def test_missing_pre_commit(self):
        config = CommitguardConfig.from_dict({"tool": {"commitguard": {"foo": "bar"}}})
    ...
    def test_get_mode_unknown(self):
        config = CommitguardConfig.from_dict({"tool": {"commitguard": {"mode": "foo"}}})
    ...
    def test_get_mode_undefined(self):
        config = CommitguardConfig.from_dict({"tool": {"commitguard": {"mode": None}}})

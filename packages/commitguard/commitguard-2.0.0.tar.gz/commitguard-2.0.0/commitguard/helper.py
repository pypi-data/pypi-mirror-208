...
from .errors import CommitGuardError
...
def download(
    ...
    *,
    ...
    destination = Path(destination) if destination else Path(url.split("/")[-1])
...
@contextmanager
def add_sys_path(directory: Union[str, os.PathLike]) -> Generator[None, None, None]:
    """
...
@contextmanager
def ensure_unload_module(module: Union[str, ModuleType]) -> Generator[None, None, None]:
    """
...
def parse_timedelta(time_str: str) -> timedelta:
    ...
    if not time_match:
        raise CommitGuardError(f"Invalid timedelta format '{time_str}'.")

    parts = time_match.groupdict()
    time_params = {name: float(param) for name, param in parts.items() if param}

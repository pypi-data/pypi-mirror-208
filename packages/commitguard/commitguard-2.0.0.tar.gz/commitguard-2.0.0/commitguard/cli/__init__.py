...
from commitguard.cli.plugins import add_plugins, list_plugins, plugins, remove_plugins
...
def main():
    ...
    plugins_parser = subparsers.add_parser("plugins", help="Manage commitguard plugins")
    plugins_parser.set_defaults(func=plugins)

    plugins_subparsers = plugins_parser.add_subparsers(dest="subcommand", required=True)

    add_plugins_parser = plugins_subparsers.add_parser("add", help="Add plugins.")
    ...
    remove_plugins_parser.add_argument("name", nargs="+", help="Plugin(s) to remove")

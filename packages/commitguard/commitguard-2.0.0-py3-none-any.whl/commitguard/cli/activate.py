...
from commitguard.config import get_pyproject_toml_path, load_config_from_pyproject_toml
...
def install_hooks(term: Terminal, args: Namespace) -> None:
    ...
    if pre_commit_hook.exists() and not args.force:
        term.warning(
            "commitguard pre-commit hook is already" f" installed at {pre_commit_hook}."
        )
        with term.indent():
            term.print()
            term.info(

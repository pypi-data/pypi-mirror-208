...
from commitguard.terminal import Progress, Terminal, _set_terminal
from commitguard.utils import get_project_commitguard_plugins_path


@contextmanager
def commitguard_module_path() -> Generator:
    plugins = get_project_commitguard_plugins_path()
...
def check_hook_is_current(term: Terminal, pre_commit_hook: PreCommitHook) -> None:
    if not pre_commit_hook.is_current_commitguard_pre_commit_hook():
        term.warning(
...
def check_hook_mode(term: Terminal, config_mode: Mode, hook_mode: Mode) -> None:
    if config_mode.get_effective_mode() != hook_mode.get_effective_mode():
        term.warning(
            f'commitguard mode "{str(hook_mode)}" in pre-commit hook differs '
...
def check_plugin(plugin_name: str) -> Optional[CheckPluginResult]:
    ...
    try:
        ...
        return CheckPluginError(
            f'"{plugin_name}" is not a valid commitguard plugin. {e}'
        )
    return None
...
def run() -> int:
    ...
    term.bold_info("commitguard => pre-commit")

    with commitguard_module_path(), term.indent(), Progress(terminal=term) as progress:
                ...
                try:
                    ...
                    if not has_precommit_function(plugin):
                        term.fail(
                            ...
                            "Your commitguard settings may be invalid."
                        )
                        return 1

...
def remove_vid_from_batch_processing_file(full_file_name, remove_files_list):
    ...
    save_old_file = (get_time_string()+'_batch_processing_old_auto_save').join(full_file_name.split('file_list_for_batch_processing'))

    save_obj(batch_f, save_old_file)
    out, inds = lister_it(batch_f['mp4_names'], keep_strings='', remove_string=remove_files_list, return_bool_index=True)
...
def load_training_and_curated_data(h5_list, labels_key, train_val_split=[7, 3], nan_ify_data=True, new_data_weight=2, set_neg_1_to_nan=True):

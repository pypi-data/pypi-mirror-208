...
def findPPDBothMethods(latitude_range,inc_deg,alt,elev_deg,constants,cos_method):
    ...
    for ii in range(len(ppd)):
        ...
        if(i <= 90):
            if(cos_method == True):
                ...
                #f_i_l = 4/math.pi/math.pi*np.arctan(lam_prime[ii]/180.0*math.pi-math.pi/2)*np.arctan(2*i/180.0*math.pi-math.pi)
                #f_i_l = np.cos(lam_prime[ii] / 180.0 * math.pi) * np.cos(i / 180.0 * math.pi)
        else:
            if(cos_method == True):
                f_i_l = np.cos(i/180.0*math.pi)
            else:

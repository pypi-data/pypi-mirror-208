...
def filelist2df(flist):
    ...
    columnname = 0
    df = pd.concat([pd.DataFrame(q) for q in flist])
    ...
    dfe = (
        pd.concat(collectionofdf, axis=1, ignore_index=True)
        .fillna(pd.NA)
    ...
    dfe.columns = [
        ...
        "aa_mime",
        "aa_encoding",
        "aa_possible_extensions",
        "aa_possible_filenames",
    ]

    return dfe


def get_file_list_from_folders(path, maxsubfolders=-1):
    return [x.path for x in get_folder_file_complete_path(path, maxsubfolders)]


def guess_filetypes(
    files_folders: list | str,
    maxsubfolders: int = -1,
    pandas_dataframe: bool = False,
    verbose: bool = True,
):
    r"""
    Guesses the file types of the provided files and folders. It uses the binaries from https://github.com/julian-r/file-windows/releases
...
result_df = guess_filetypes(
    ...
    """
    if not isinstance(files_folders, list):
        files_folders = [files_folders]
    allfi = []
    for f in files_folders:
        if os.path.isdir(f) or os.path.ismount(f):
            allfi.extend(get_file_list_from_folders(f, maxsubfolders=maxsubfolders))
        if os.path.isfile(f):
            allfi.append(f)
    ...
    for fi in allfi:
        ...
        p = subprocess.run(command, capture_output=True, shell=True, **invisibledict)
        resu2 = re.findall(
            rb"(^[^\n\r]+)\xc7\xc7\xc7\xc7\s+([^\n\r]+);\s+([^\n\r]+)", p.stdout
        )
        allresults.append(resu2)
            ...
            [
                [
                    [
                        ...
                        g := x[1].decode("utf-8"),
                        x[2].decode("utf-8"),
                        m := tuple(mimesreversed.get(g, [])),
                        tuple([a + "." + mm for mm in m])
                        if not pathlib.Path(a).suffix.strip(".").lower() in m
                        else (a,),
                    ]
                    for x in resu2
                ]
    ...
    if pandas_dataframe:
        try:
            return filelist2df(allresults2)
        except Exception as fe:
            print(fe)
            print(
                "pandas could not be found. (pip install pandas)\nthe results will be returned as a list"
            )
    return allresults2

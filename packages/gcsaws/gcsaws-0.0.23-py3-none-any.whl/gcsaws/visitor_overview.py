...
class VisitorStateOverview:
    ...
    def _capture(self) -> Console:
        ...
        console = Console(record=True, width=self.width, soft_wrap=True)
...
def visit_composed_node(visitor: Visitor,
    ...
    current = state.current_tree.add(assemble_lines(title, make_description(node)), guide_style=guide_style)

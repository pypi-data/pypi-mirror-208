...
#        deployment_spec = k8s_client.ExtensionsV1beta1DeploymentSpec(
    ...
    def delete (self, name, namespace="default"):
        ...
        try: 
            ...
            logger.debug(f"Start traceback print exc=============================================================")
            traceback.print_exc()
            logger.debug(f"End traceback print exc===============================================================")
            exc_type, exc_value, exc_traceback = sys.exc_info()
            logger.debug(f"exc type ---> {exc_type}, exc value ---> {exc_value}, exc_traceback ---> {exc_traceback}")
            text = traceback.format_exception(
                exc_type, exc_value, exc_traceback)
            logger.debug(f"TEXT and RAISED DELETE EXCEPTION ======================================> {text}")
            raise DeleteException (
                message=f"Failed to delete system: {name}",
                details=text)
            logger.debug(f"End DELETE EXCEPTION =================================================================")
        return {
        }

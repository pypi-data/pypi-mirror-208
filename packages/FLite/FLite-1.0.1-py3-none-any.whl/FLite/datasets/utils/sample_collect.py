...
from FLite.datasets.utils.splitdata_simulation import non_iid_class
from FLite.datasets.utils.constants import SEED_FILES
from FLite.datasets.utils.save_load_util import iid_divide
...
def extreme(
    ...
    clients, all_user_data = non_iid_class(
        x_list, y_list, class_per_client, num_of_client
    )
...
def sample(
    data_dir, data_folder, metafile, fraction, iid, iid_user_fraction=0.01, seed=-1
):
    logger.info("------------------------------")
    logger.info("sampling data")
    subdir = os.path.join(data_dir, "all_data")

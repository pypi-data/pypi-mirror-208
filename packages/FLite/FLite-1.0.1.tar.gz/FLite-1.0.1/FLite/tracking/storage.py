...
from FLite.protocol.codec import marshal

logger = logging.getLogger(__name__)

DEFAULT_SQLITE_DB = "FLite.db"
...
class SqliteStorage(object):
    ...
    def store_round_metric(self, metric):
        ...
        try:
            ...
            logger.error(
                f"Failed to store round metric {metric.task_id} {metric.round_id}, error: {e}"
            )
    ...
    def store_client_train_metric(
        ...
        sql = (
            "INSERT INTO client_metric (task_id, round_id, client_id, train_loss, train_time, "
    ...
    def store_client_test_metric(
        ...
        sql = (
            "UPDATE client_metric SET test_accuracy=?, test_loss=?, test_time=? ,test_upload_size=?, "
    ...
    def get_round_metrics(self, task_id, rounds):
        if rounds:
            sql = (
                "SELECT * FROM round_metric WHERE task_id=? AND round_id IN (%s)"
                % ("?," * len(rounds))[:-1]
            )
    ...
    def get_client_metrics(self, task_id, round_id, client_ids=None):
        if client_ids:
            sql = (
                "SELECT * FROM client_metric WHERE task_id=? AND round_id=?  \
                   AND client_id IN (%s)"
                % ("?," * len(client_ids))[:-1]
            )
    ...
    def delete_round_metric(self, task_id, round_id):
        ...
        try:
            ...
            logger.error(
                f"Failed to delete round metric {task_id} {round_id}, error: {e}"
            )
...
class RemoteStorage(object):
    ...
    def store_task_metric(self, metric):
        response = self.tracking_stub.TrackTaskMetric(
            tracking_pb.TrackTaskMetricRequest(task_metric=metric.to_proto())
        )
    ...
    def store_round_metric(self, metric):
        ...
        if response.status == common_pb.SC_UNKNOWN:
            logger.error(
                f"Failed to store round metric, task_id: {metric.task_id} round_id: {metric.round_id}."
            )
        return response.status
    ...
    def store_client_train_metric(
        ...
        req = tracking_pb.TrackClientTrainMetricRequest(
            task_id=tid,
            round_id=rid,
            client_id=cid,
        ...
        if response.status == common_pb.SC_UNKNOWN:
            logger.error(
                "Failed to store client metric, task id: {} round id: {} client id: {}.".format(
                    tid, rid, cid
                )
            )
        return response.status
    ...
    def store_client_test_metric(
        ...
        req = tracking_pb.TrackClientTestMetricRequest(
            task_id=tid,
            round_id=rid,
            client_id=cid,

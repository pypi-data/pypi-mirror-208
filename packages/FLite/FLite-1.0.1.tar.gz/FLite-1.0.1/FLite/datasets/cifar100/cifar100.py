...
from FLite.datasets.utils.splitdata_simulation import data_simulation
from FLite.datasets.BaseDatasets import BaseDataset, CIFAR100
from FLite.datasets.utils.save_load_util import save_dict
...
class Cifar100(BaseDataset):
    ...
    def download_raw_file_and_extract(self):
        train_set = torchvision.datasets.CIFAR100(
            root=self.base_folder, train=True, download=True
        )
        test_set = torchvision.datasets.CIFAR100(
            root=self.base_folder, train=False, download=True
        )

        self.train_data = {"x": train_set.data, "y": train_set.targets}

        self.test_data = {"x": test_set.data, "y": test_set.targets}

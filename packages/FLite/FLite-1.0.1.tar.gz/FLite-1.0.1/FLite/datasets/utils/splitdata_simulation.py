...
def non_iid_dirichlet(
    data_x, data_y, num_of_clients, alpha, min_size, x_dtype, y_dtype
):
    ...
    while current_min_size < min_size:
        ...
        for k in range(num_class):
            ...
            proportions = np.array(
                [
                    p * (len(idx_j) < data_size / num_of_clients)
...
def non_iid_class(
    data_x, data_y, class_per_client, num_of_clients, x_dtype, y_dtype, stack_x=True
):
    ...
    for i in clients:
        for j in range(class_per_client):
            ...
            if len(federated_data[i]["x"]) != 0:
                if stack_x:
                    federated_data[i]["x"] = np.vstack(
                        (federated_data[i]["x"], data_x[part_indexes])
                    ).astype(x_dtype)
                else:
                    federated_data[i]["x"] = np.append(
                        federated_data[i]["x"], data_x[part_indexes]
                    ).astype(x_dtype)
                federated_data[i]["y"] = np.append(
                    federated_data[i]["y"], data_y[part_indexes]
                ).astype(y_dtype)
            else:
...
def data_simulation(
    ...
    if data_distribution == SIMULATE_IID:
        group_client_list, group_federated_data = iid(
            data_x, data_y, num_of_clients, x_dtype, y_dtype
        )
    elif data_distribution == SIMULATE_NIID_DIR:
        group_client_list, group_federated_data = non_iid_dirichlet(
            data_x, data_y, num_of_clients, alpha, min_size, x_dtype, y_dtype
        )
    elif data_distribution == SIMULATE_NIID_CLASS:
        group_client_list, group_federated_data = non_iid_class(
            data_x,
            data_y,
            class_per_client,
            num_of_clients,
    ...
    for i in group_client_list:
        ...
        input_lists, label_lists = quantity_hetero(
            weights, current_client["x"], current_client["y"]
        )

...
def create_system(ID='corn_sys', flowsheet=None):
    ...
    CSL_ferm = bst.Stream('CSL_ferm', 
                          # price=price['CSL'],
                          price = 0.,
                          units='kg/hr')
    DAP_ferm = bst.Stream('DAP_ferm', 
                          # price=price['DAP'],
                          price = 0.,
                          units='kg/hr')
    air_fresh = bst.Stream('air_fresh', O2=21., N2=79., price=price['Compressed air'], units='kg/hr',
                           phase='g')
    ...
    K401 = bst.IsentropicCompressor('K401', ins=air_fresh, outs=('compressed_air',), 
                                    ...
                                    eta=0.75,
                                    vle=False)
    H401 = bst.HXutility('H401', ins=K401-0, outs=('cooled_air'), T=32.+273.15)
    # @K401.add_specification(run=True)
    # def K401_spec():
    #     V405._run()
    #     K401.ins[0].mol[:] = K401.outs[0].mol[:]
    #     # K401.show()
    V405 = blst.bluesteam_biorefineries.generic.units.SSF('V405', (E402-0, P404-0, CSL_ferm, DAP_ferm, H401-0, 'dilution_water'), outs=('CO2', ''), V=1.9e3)

...
class Color:
    ...
    def __init__(self, main_color: str = None, background_color: Optional[str] = None) -> None:
        """
        Class attributes can be used without initialization
        Otherwise, main_color is required to use functions
        """
        ...
        if not main_color or main_color not in fg_colors:
            raise AttributeError(f"main_color required (available: {', '.join(fg_colors)}")
        ...
        self.char_map = [v for v in Color.__dict__.values()
    ...
    def colorize(self, text: str = None, color: Optional[str] = None,
        ...
        fg_fragment = f"{self.__getattribute__(color if color else self.main_color)}"
        bg_fragment = ""
        if background:
            bg_fragment = self.__getattribute__(background)
        elif self.background_color:
            bg_fragment = self.__getattribute__(self.background_color)
...
class MenuEntry:
    ...
    def __init__(self, func=None, title: str = None, show_signature: bool = True, **kwargs) -> None:
        self.func = func
        self.title = title
    ...
    menu=[
        ...
        while True:
            ...
            for idx, entry in enumerate(menu):
                title = f"{c.create_index(idx + 1)} {c.colorize('Title:')}"
                header_just = c.ljustify(f"{c.create_index(idx + 1)} {c.colorize('Title:')}",
                                         min_len)

                # iterate over arguments within MenuEntry
                entries += f"{header_just} {entry.title}"

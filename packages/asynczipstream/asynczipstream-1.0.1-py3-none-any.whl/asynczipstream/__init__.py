...
class PointerIO(object):
    def __init__(self, mode="wb"):
        if mode not in ("wb",):
            raise RuntimeError('asynczipstream.ZipFile() requires mode "wb"')
        self.data_pointer = 0
        self.__mode = mode
        self.__closed = False
...
class ZipFile(zipfile.ZipFile):
    def __init__(
        ...
        if mode not in ("w",):
            raise RuntimeError('asynczipstream.ZipFile() requires mode "w"')
        if fileobj is None:
            fileobj = PointerIO()
    ...
    def write_iter(
        self, arcname, iterable, compress_type=None, buffer_size=None, date_time=None
    ):
    ...
    def writestr(
        self, arcname, data, compress_type=None, buffer_size=None, date_time=None
    ):
        """

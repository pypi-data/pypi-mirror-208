...
class TestPyGuppyClient(unittest.TestCase):
    ...
    def test_04_pass_reads_and_get_completed(self):
        ...
        client.set_params(
            {"client_name": "guppy_client_test_04_pass_reads_and_get_completed"}
        )
        client.connect()
        self.assertEqual(client.get_status(), PyGuppyClient.connected)
    ...
    def test_05_pass_read_malformed(self):
        ...
        client.set_params({"client_name": "guppy_client_test_05_pass_read_malformed"})
        client.connect()
        self.assertEqual(client.get_status(), PyGuppyClient.connected)
    ...
    def test_06_pass_read_connection_error(self):
        ...
        client.set_params(
            {"client_name": "guppy_client_test_06_pass_read_connection_error"}
        )
    ...
    def test_07_get_completed_reads_raises(self):
        ...
        client.set_params(
            {"client_name": "guppy_client_test_07_get_completed_reads_raises"}
        )
    ...
    def test_08_context_manager(self):
        ...
        client.set_params({"client_name": "guppy_client_test_08_context_manager"})
    ...
    def test_09_set_params_already_connected(self):
        ...
        client.set_params(
            {"client_name": "guppy_client_test_09_set_params_already_connected"}
        )
    ...
    def test_10_set_params(self):
        ...
        client.set_params({"client_name": "guppy_client_test_10_set_params"})
    ...
    def test_11_config_with_suffix(self):
        ...
        client.set_params({"client_name": "guppy_client_test_11_config_with_suffix"})
        client.connect()
        self.assertEqual(client.get_status(), PyGuppyClient.connected)
    ...
    def test_12_get_server_stats(self):
        ...
        client.set_params({"client_name": "guppy_client_test_12_get_server_stats"})
        client.connect()
        self.assertEqual(client.get_status(), PyGuppyClient.connected)
    ...
    def test_14_return_code_messages(self):
    ...
    def test_15_pass_reads_override_and_get_completed(self):
        if TestPyGuppyClient.SERVER_ADDRESS is None:
            raise unittest.SkipTest("No server specified")
        if H5PY_UNAVAILABLE:
            raise unittest.SkipTest("h5py unavailable")
        ...
        client.set_params(
            {
                "client_name": "guppy_client_test_15_pass_reads_override_and_get_completed"
            }
        )
        client.connect()
        self.assertEqual(client.get_status(), PyGuppyClient.connected)
        ...
        while recv < sent:
            ...
            for split_read in reads:
                for read in split_read:
                    recv_ids.add(read["metadata"]["read_id"])
                    sequences1[read["metadata"]["read_id"]] = read["datasets"][
                        "sequence"
                    ]
            time.sleep(0.1)
        ...
        # second run, with scaling overrides
        sequences2 = dict()
        for single_read_file in input_folder.glob("*.fast5"):
            with get_fast5_file(single_read_file, mode="r") as f5:
                f5_read = f5.get_read(f5.get_read_ids()[0])
                read = package_read(**pull_read(f5_read))
                read["override_scaling"] = True
                read["scaling_med"] = read["daq_scaling"] * 2
                read["scaling_mad"] = read["daq_offset"] * 2
            status = client.pass_read(read)
            if status:
                sent += 1
                sent_ids.add(read["read_id"])
            ...
            for split_read in reads:
                for read in split_read:
                    ...
                    self.assertAlmostEqual(
                        read["metadata"]["daq_scaling"] * 2,
                        read["metadata"]["scaling_median"],
                        4,
                    )
                    self.assertAlmostEqual(
                        read["metadata"]["daq_offset"] * 2,
                        read["metadata"]["scaling_med_abs_dev"],
                        4,
                    )
                    sequences2[read["metadata"]["read_id"]] = read["datasets"][
                        "sequence"
                    ]
            time.sleep(0.1)
        ...
        # check that the scaling overrides have given us different results
        self.assertEqual(sequences1.keys(), sequences2.keys())
        self.assertNotEqual(sequences1, sequences2)

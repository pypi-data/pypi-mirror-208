...
def pull_read(fast5_read):
    ...
    if not isinstance(input_read, Fast5Read):
        warnings.warn(
            "pull_read() will no longer support pathnames soon.", DeprecationWarning
        )
...
def send_reads_with_pyguppy(client, input_path, batch_size=1):
    files = [
    ...
    while num_reads_sent < read_count:
        ...
        if len(reads) == 1:
            result = client.pass_read(reads[0])
        else:
            result = client.pass_reads(reads)
...
def run_server(options, bin_path=None):
    ...
    """
    if DEBUG:
        executable = "guppy_basecall_serverd"
    else:

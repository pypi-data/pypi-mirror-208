...
class FeatureDefinitionAndJoinConfig:
    ...
    # Note: it returns a tuple that contains (Tecton schema, Spark schema). The Spark schema is from request context
    # source, and we are not able to convert it to Tecton schema because we can't import `tecton_spark` package in
    # `tecton_score`.
    ...
    def spine_schema(self) -> Tuple[Schema, Optional["SparkSchema"]]:
        (fd_jk_schema, rc_schema) = self.feature_definition.spine_schema
        fd_jk_schema_dict = fd_jk_schema.to_dict()
        jk_schema_dict = {
            spine_key: fd_jk_schema_dict[fd_key]
            for spine_key, fd_key in self.join_keys
            if fd_key != self.feature_definition.wildcard_join_key
        }
        return (Schema.from_dict(jk_schema_dict), rc_schema)

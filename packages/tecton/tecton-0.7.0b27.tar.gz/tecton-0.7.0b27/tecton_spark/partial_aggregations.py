...
def construct_partial_time_aggregation_df(
    ...
    if not time_aggregation.is_continuous:
        ...
        columns_to_drop = set()
        for feature in time_aggregation.features:
            column_prefixes = get_materialization_aggregation_column_prefixes(feature.function, is_continuous=True)
            column_names = [f"{column_prefix}_{feature.input_feature_name}" for column_prefix in column_prefixes]
            aggregation_plan = get_aggregation_plan(
                feature.function, feature.function_params, time_aggregation.is_continuous, time_aggregation.time_key
            )
            continuous_columns = aggregation_plan.continuous_partial_aggregation_transform(feature.input_feature_name)

            for column_name, column in zip(column_names, continuous_columns):

"""
GUI for beaglebone GPIO server.
...
class BBGPIOWidget(QtWidgets.QWidget):
    """Qt widget for controlling the Beaglebone Black GPIO server."""
    def __init__(self, bbgpio):
        """
        Args:
            bbgpio: The Beaglebone Black GPIO driver.
        """
        super().__init__()
        ...
        self.bbgpio.pin_state_changed.connect(self.pin_changed_external)
        for i, pin in enumerate(self.bbgpio.pins):
    ...
    def combobox_changed(self, pin, index):
        ...
        self.bbgpio.set_pin(pin, index, emit=False)

...
class chromaticRV():
    ...
    def get_rv(self, orders):
        ...
        # else:
        return list(map(np.array, (rv, rve, fwhm, fwhme)))
    ...
    @property
    def _blue_arm_RV(self):
        """
        Calculate RVs for the blue arm only: orders 0 ... 90 (exc)
        Returns RV, RV errors, FWHM, FWHM errors
        """
        blue_arm_orders = slice(0, 90, 1)
        out = self.get_rv(blue_arm_orders)
        return list(out)

    @property
    def _red_arm_RV(self):
        """
        Calculate RVs for the red arm only: orders 90 (inc) ... 170 (exc)
        Returns RV, RV errors, FWHM, FWHM errors
        """
        red_arm_orders = slice(90, 170, 1)
        out = self.get_rv(red_arm_orders)
        return list(out)

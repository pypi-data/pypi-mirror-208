...
def gauss(x: array, p: array) -> np.ndarray:
    """ A Gaussian function with parameters p = [A, x0, σ, offset]. """
    return p[0] * exp(-(x - p[1])**2 / (2 * p[2]**2)) + p[3]


def _gauss_partial_deriv(x: array, p: array) -> np.ndarray:
...
def _gauss_initial_guess(x: np.ndarray, y: np.ndarray) -> List:
...
def gaussfit(x: np.ndarray,
    ...
    if yerr is None:
        def f(p: np.ndarray, x: np.ndarray, y: np.ndarray) -> np.ndarray:
            return gauss(x, p) - y
    else:

        def f(p: np.ndarray, x: np.ndarray, y: np.ndarray,
              yerr: np.ndarray) -> np.ndarray:
            return (gauss(x, p) - y) / yerr
    ...
    pfit, pcov = optimize.curve_fit(
        f, x, y, p0=p0, sigma=yerr, jac=df, **kwargs)

    ...
    def add_read_info(self, tagged_read: dict,
        ...
        if status == 0:
            ...
            qbed_record = {'chr': tagged_read['read'].reference_name,
                           ...
                           'end': tagged_read['read'].get_tag('XI') + insert_offset,  # noqa
                           'strand': '+' if tagged_read['read'].is_forward else '-',  # noqa
                           'annotation': annotation}
            self._qbed_writer.writerow(qbed_record)
        # add barcode qc data
        if record_barcode_qc:
            qc_read_keys = ['query_name', 'mapping_quality']
            qc_read_info = {k: getattr(
                tagged_read['read'], k) for k in qc_read_keys}
            qc_read_info['read_id'] = self._query_id
            qc_read_info['status'] = status
            for component, comp_dict in \
                    tagged_read['barcode_details']['details'].items():
    ...
    def to_qbed(self, output: str, barcode_qc: bool = True) -> pd.DataFrame:  # noqa
        ...
        if len(df) == 0:
            ...
            df = df\
                ...
                .size()\
                .reset_index()\
                .rename(columns={0: 'depth'})[['chr', 'start', 'end',
                                               'depth', 'strand',
                                               'annotation']]
            ...
            if output:
                if os.path.exists(output):
                    raise FileExistsError(
                        f'{output} already exists -- cannot overwrite')
                else:
                ...
                if barcode_qc:
                    annote_tally_df = df\
                        ...
                        .size()\
                        .reset_index()\
                        .rename(columns={0: 'tally'})[['annotation', 'tally']]
                    annote_tally_df\
                        .to_csv(
                    ...
                    multi_bc_sites = df\
                        ...
                        .size()\
                        .reset_index()\
                        .rename(columns={0: 'tally'})[['barcode_count_by_site',
                                                       'tally']]
                    multi_bc_sites\
                        .to_csv(

    ...
    def fit(self,  epochs=None,  samples=None,
        ...
        for epoch in range(1, epochs+1):
            ...
            if self.folders.point and 'point' in monitor and (period_point > 0 and point_start < self.hist.epochs and self.hist.epochs % period_point == 0 or epoch == epochs):
                ...
                self.save(Path(self.folders.point) / Path(self.folders.prefix + f"point_{self.now()}_score_val_{score_val[0]:.4f}_trn_{score_trn[0]:.4f}_loss_val_{loss_val:.4f}_trn_{loss_trn:.4f}.pt"), model=self.model, optim=self.optim)

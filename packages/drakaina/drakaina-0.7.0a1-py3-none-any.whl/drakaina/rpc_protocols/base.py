...
class BaseRPCProtocol:
    ...
    :type schema_serializer: BaseSerializer
    :param openrpc_schema_template: todo
    :type openrpc_schema_template: OpenRPC | dict
    ...
    def __init__(
        ...
        openrpc_schema_template: OpenRPC | None = None,
        openapi_schema_template: OpenAPI | None = None,
    ):
        ...
        self.openrpc_schema_template = (
            openrpc_schema_template
            if openrpc_schema_template is not None
            else DEFAULT_OPENRPC_SCHEMA
        )
        self.__openrpc_schema = None
        self.openapi_schema_template = (
            openapi_schema_template
            if openapi_schema_template is not None
            else DEFAULT_OPENAPI_SCHEMA
        )
    ...
    def openrpc_schema(self) -> OpenRPC:
        ...
        if not self.__openrpc_schema:
            schema = self.openrpc_schema_template.copy()
            ...
            for method_name, procedure_ in self.registry.items():
                ...
                for param_name, parameter in parameters.items():
                    ...
                    if "summary" in parameter:
                        cd_param["summary"] = parameter["summary"]
                ...
                if "result" in method_schema:
                    ...
                    if "summary" in result_schema:
                        cd_result["summary"] = result_schema["summary"]

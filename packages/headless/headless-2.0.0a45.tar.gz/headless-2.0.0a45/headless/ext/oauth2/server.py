...
class Server:
    ...
    """
    _metadata: ServerMetadata | None
    _metadata_cache: dict[str, ServerMetadata] = {}
    ...
    def metadata(self) -> ServerMetadata | None:
        return self._metadata or Server._metadata_cache.get(self.client.base_url)

    @metadata.setter
    def metadata(self, value: ServerMetadata) -> None:
        if getattr(self.client, 'base_url', None):
            Server._metadata_cache[self.client.base_url] = value
        else:
            # Assume that metadata is defined by the constructor.

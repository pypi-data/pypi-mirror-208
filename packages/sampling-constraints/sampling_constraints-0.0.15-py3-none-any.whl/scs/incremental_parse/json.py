...
class JSONParser(IncrementalParser):
    ...
    def __init__(
        ...
        self._allow_consecutive_whitespace = allow_consecutive_whitespace
        self._subparser = None
        self._complete = False
        self._previous_whitespace = False
    ...
    def _copy_from(self, other: "JSONParser"):
        ...
        self._allow_consecutive_whitespace = other._allow_consecutive_whitespace
        self._previous_whitespace = other._previous_whitespace
    ...
    def _append(self, char: Union[str, SpecialToken]) -> bool:
        if self._subparser is None:
            if char == SpecialChar.OPEN_ARRAY.value:
                if self._allow_outer_list:
                    self._subparser = ArrayParser(allow_empty=self._allow_empty, allow_empty_children=self._allow_empty_children)
                else:
                    raise ParseFailure("Only allow object in outer JSON")
            elif char == SpecialChar.OPEN_OBJECT.value:
                self._subparser = ObjectParser(allow_empty=self._allow_empty, allow_empty_children=self._allow_empty_children)
        ...
        if char.isspace():
            if self._previous_whitespace and not self._allow_consecutive_whitespace:
                raise ParseFailure("Got consecutive whitespaces. If expected set allow_consecutive_whitespace accordingly.")
            self._previous_whitespace = True
            return False
        self._previous_whitespace = False
...
class ObjectParser(IncrementalParser):
    ...
    def __init__(self, allow_empty: bool = True, allow_empty_children: bool = True):
        super().__init__()
        self._allow_empty = allow_empty
        self._allow_empty_children = allow_empty_children
    ...
    def _open_subparser(self, char: str):
        if char == SpecialChar.OPEN_OBJECT.value:  # begin parsing object
            self._active_subparser = ObjectParser(allow_empty=self._allow_empty_children, allow_empty_children=self._allow_empty_children)
        elif char == SpecialChar.OPEN_ARRAY.value:  # begin parsing array
            self._active_subparser = ArrayParser(allow_empty=self._allow_empty_children, allow_empty_children=self._allow_empty_children)
    ...
    def _append(self, char: str) -> bool:
        ...
        if char.isspace():
            raise ParseFailure("Spaces outside values is disallowed")
        
        if self._parse_status == ObjectParseStatus.OPENED:
            if char == SpecialChar.CLOSE_OBJECT.value:
                if not self._allow_empty:
                    raise ParseFailure("Got empty object. If this is expected set allow_empty and allow_empty_children accordingly")
                self._parsed += char
                return True
...
class ArrayParser(ObjectParser):
    ...
    def __init__(self, allow_empty: bool = True, allow_empty_children: bool = True):
        super().__init__(allow_empty=allow_empty, allow_empty_children=allow_empty_children)

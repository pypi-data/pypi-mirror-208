...
def entity_text_annotation_to_json(text_annotation: str) -> Dict[str, Any]:
    blob = {
        'text': '(<TIME>0:26</TIME> - <PERIOD>2nd</PERIOD>) Timeout #3 by <TEAM>MIA</TEAM> at <TIME>00:26</TIME>.',
        'entities': []
    }
...
def save_entities() -> None:
    blobs = [
        ...
        for text in load_data(os.path.join(WORKSPACE, '3', 'dev.txt'))
    ]
    ...
    save_data(
        ...
        list(map(lambda item: item['sentence'], dataset))
    )

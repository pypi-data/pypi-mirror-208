...
class FastAPIExtended(FastAPI):
    ...
    def get_setting(self) -> FastApiConfig:
        result: FastApiConfig = None
        for obj in self.features:
            if FastApiConfig in obj.__class__.__mro__:
                result = obj
        if result is None:
    ...
    def dependency_lookup(self) -> None:
        ...
        """
        if self.config.database_url and not self.db_engine:
            raise DependencyException("Pass db_engine if you wish to use PG")
        for feature in self.features:

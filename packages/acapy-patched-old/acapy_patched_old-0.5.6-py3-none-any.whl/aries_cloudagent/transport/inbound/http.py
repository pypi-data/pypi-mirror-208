...
class HttpTransport(BaseInboundTransport):
    ...
    async def make_application(self) -> web.Application:
        ...
        app.add_routes([web.get("/.well-known/agent-configuration",
                                self.connections_create_agent_invitation)])
        app.add_routes([web.post("/.well-known/agent-configuration",
                                 self.connections_create_agent_invitation)])
    ...
    async def invite_message_handler(self, request: web.BaseRequest):
        ...
        """
        raise web.HTTPFound("/.well-known/agent-configuration")

    async def connections_create_agent_invitation(self, request: web.BaseRequest):
        """
        Request handler for creating a new mediator connection invitation.
        ...
        session = await self.create_session(
            accept_undelivered=True, can_respond=False, client_info=client_info
        )

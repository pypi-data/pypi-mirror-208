...
from ..valid import INDY_CRED_DEF_ID, INDY_REV_REG_SIZE, INDY_SCHEMA_ID, INDY_VERSION, UUIDFour, IndyDID
...
class CredentialDefinitionDetailSchema(OpenAPISchema):
    """Credential definition detail schema."""
    ...
    schema_id = fields.Str(
        description="Schema identifier within credential definition identifier",
        example="9DkTAmKAVVQ47RcKDTzevG:2:Covid19 Test Result:1.0"
    )
    schema_issuer_did = fields.Str(
        description="Schema issuer DID",
        example="9DkTAmKAVVQ47RcKDTzevG",
    )
    ...
    cred_def_id = fields.Str(
        description="Credential definition identifier",
        example="9DkTAmKAVVQ47RcKDTzevG:3:CL:14:default",
    )
    schema_attributes = fields.List(
        fields.Str(description="Schema attribute name", example="abc")
    )
    epoch = fields.Str(
...
class CredentialDefinitionsCreatedResultsSchema(OpenAPISchema):
    """Results schema for credential definitions created."""

    results = fields.List(fields.Nested(CredentialDefinitionDetailSchema()),
                          description="List of credential definition records")
...
@response_schema(CredentialDefinitionDetailSchema(many=True), 200)
async def credential_definitions_created(request: web.BaseRequest):
    ...
    # Sort result by epoch
    if results:
        results = sorted(results, key=lambda x: ("epoch" in x, x.get('epoch', None)), reverse=True)
        results.reverse()
...
@docs(
    tags=["credential-definition"],
    summary="Deletes a credential definition that is agent originated",
)
@match_info_schema(CredDefIdMatchInfoSchema())
async def credential_definitions_delete_credential_definition(request: web.BaseRequest):
    """
    Request handler for deleting a credential definition record from the wallet.
    ...
    if found:
        ...
        raise web.HTTPNotFound(reason="Credential definition record not found")
...
async def register(app: web.Application):
    ...
    app.add_routes(
        [
            ...
            web.delete(
                "/credential-definitions/{cred_def_id}",
                credential_definitions_delete_credential_definition,
            ),
        ]
    )

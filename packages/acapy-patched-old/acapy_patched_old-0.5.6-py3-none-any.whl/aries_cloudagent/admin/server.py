...
HTTP_HEADER_API_KEY = "Authorization"
...
class AdminServer(BaseAdminServer):
    ...
    def __init__(
        ...
        """
        self.app = None
        self.igrantio_api_key = context.settings.get("igrantio_api_key")
        self.igrantio_org_id = context.settings.get("igrantio_org_id")
    ...
    async def make_application(self) -> web.Application:
        ...
        def is_unprotected_path(path: str):
            return (
                ...
                or path.startswith("/igrantio-operator/connection/qr-link/")
                or path.startswith("/igrantio-operator/data-exchange/qr-link/")
            )

        # Print iGrant.io operator configuration for debugging
        print("\n\n\n\n")
        print("iGrant.io operator configuration")
        print("================================")
        print("iGrant.io API Key         : \t",
              self.context.settings.get("igrantio_api_key"))
        print("iGrant.io Organization ID : \t",
              self.context.settings.get("igrantio_org_id"))
        print("iGrant.io endpoint        : \t",
              self.context.settings.get("igrantio_endpoint"))
        print("\n\n\n\n")

        # If igrantio_api_key is None, then admin_insecure_mode must be set so
        # we can safely enable the admin server with no security
        if self.igrantio_api_key and (not self.admin_insecure_mode):
            ...
            async def check_token(request, handler):
                ...
                header_admin_api_key = request.headers.get(HTTP_HEADER_API_KEY)

                if not header_admin_api_key:
                    raise web.HTTPUnauthorized()

                # Validate API key using JWK
                try:
                    if jwt.decode(header_admin_api_key.split("ApiKey ")[1], "iGrant.io APIKey", algorithms=["HS256"]):
                        self.context.settings["igrantio_api_key"] = header_admin_api_key.split("ApiKey ")[1]
                        return await handler(request)
                    else:
                        raise web.HTTPUnauthorized()
                except jwt.exceptions.InvalidAudienceError as e:
                    if jwt.decode(header_admin_api_key.split("ApiKey ")[1], "iGrant.io APIKey", algorithms=["HS256"], audience="dataverifier"):
                        self.context.settings["igrantio_api_key"] = header_admin_api_key.split("ApiKey ")[1]
                        return await handler(request)
                    else:
                        raise web.HTTPUnauthorized()
    ...
    async def on_startup(self, app: web.Application):
        ...
        if self.igrantio_api_key:
            swagger = app["swagger_dict"]
            swagger["securityDefinitions"] = {
                "ApiKeyHeader": {"type": "apiKey", "in": "header", "name": HTTP_HEADER_API_KEY}
            }
    ...
    async def websocket_handler(self, request):
        ...
        if self.admin_insecure_mode:
            ...
            valid_key = True if jwt.decode(header_admin_api_key.split("ApiKey ")[1], "iGrant.io APIKey") else False

            # authenticated via http header?
            queue.authenticated = valid_key
                    ...
                    if receive.done():
                        if not closed:
                            ...
                            try:
                                ...
                                msg_api_key = msg_received.get(HTTP_HEADER_API_KEY)
                            except Exception:
                                LOGGER.exception(
                                    "Exception in websocket receiving task:"
                                )

                            valid_key = True if msg_api_key.startswith(
                                "ApiKey ") and self.igrantio_api_key == msg_api_key.split("ApiKey ")[1] else False

                            if self.igrantio_api_key and valid_key:

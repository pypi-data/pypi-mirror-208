...
from ..server import AdminServer, AdminSetupError, HTTP_HEADER_API_KEY
...
class TestAdminServer(AsyncTestCase):
    ...
    async def test_visit_secure_mode(self):
        ...
        async with self.client_session.get(
            f"http://127.0.0.1:{self.port}/status", headers={HTTP_HEADER_API_KEY: "wrong-key"}
        ) as response:
            assert response.status == 401

        async with self.client_session.get(
            f"http://127.0.0.1:{self.port}/status",
            headers={HTTP_HEADER_API_KEY: "test-api-key"},
        ) as response:
            assert response.status == 200

        async with self.client_session.ws_connect(
            f"http://127.0.0.1:{self.port}/ws", headers={HTTP_HEADER_API_KEY: "test-api-key"}
        ) as ws:
            result = await ws.receive_json()
            assert result["topic"] == "settings"

...
async def wallet_config(context: InjectionContext, provision: bool = False):
    ...
    public_did = await wallet.create_local_did()
    if public_did:
        # write it to indy ledger
        async with aiohttp.ClientSession() as session:
            async with session.post('https://indy.igrant.io/register?ca=true', json={"verkey": public_did.verkey, "did": public_did.did, "role": "TRUSTEE" if context.settings.get("ada_mode") else "ENDORSER"}) as resp:
                if resp.status == 200 or resp.status == 201:
                    print("\n\n\n")
                    print("Public DID        : ", public_did.did)
                    print("Public DID verkey : ", public_did.verkey)
                    print("\n\n\n")

                    # make it as public
                    public_did_info = await wallet.set_public_did(public_did.did)

...
@group(CAT_START)
class IGrantIOGroup(ArgumentGroup):
    """iGrant.io settings."""

    GROUP_NAME = "IGrantIO"

    def add_arguments(self, parser: ArgumentParser):
        """Add iGrant.io operator specific command line arguments to the parser."""
        parser.add_argument(
            "-iep",
            "--igrantio-endpoint",
            type=str,
            metavar="<endpoint>",
            env_var="IGRANTIO_ENDPOINT",
            help="Specifies the iGrant.io operator endpoint for this agent.",
        )
        parser.add_argument(
            "-iak",
            "--igrantio-api-key",
            type=str,
            metavar="<apikey>",
            env_var="IGRANTIO_API_KEY",
            help="Specifies the iGrant.io operator API Key for this agent.",
        )
        parser.add_argument(
            "-iorgid",
            "--igrantio-organization-id",
            type=str,
            metavar="<orgid>",
            env_var="IGRANTIO_ORG_ID",
            help="Specifies the iGrant.io organization ID for this agent",
        )
        parser.add_argument(
            "--ada-mode",
            action="store_true",
            env_var="ADA_MODE",
            help="Enables ADA mode",
        )

    def get_settings(self, args: Namespace):
        """Extract iGrant.io operator settings."""
        settings = {}

        if args.igrantio_api_key:
            settings["igrantio_api_key"] = args.igrantio_api_key
        if args.igrantio_organization_id:
            settings["igrantio_org_id"] = args.igrantio_organization_id
        if args.igrantio_endpoint:
            settings["igrantio_endpoint"] = args.igrantio_endpoint
        if args.ada_mode:
            settings["ada_mode"] = True

        return settings


@group(CAT_START)
class MobileAgentMediatorGroup(ArgumentGroup):
    """Mobile agent mediator settings."""

    GROUP_NAME = "MobileAgentMediator"

    def add_arguments(self, parser: ArgumentParser):
        """Add mobile agent mediator specific command line arguments to the parser."""
        parser.add_argument(
            "--mobile-agent-mediator-mode",
            action="store_true",
            env_var="MEDIATOR_MODE",
            help="Specifies whether agent is started as mobile agent mediator",
        )

    def get_settings(self, args: Namespace):
        """Extract mobile agent mediator settings."""
        settings = {}

        settings["mobile_agent_mediator_mode"] = args.mobile_agent_mediator_mode

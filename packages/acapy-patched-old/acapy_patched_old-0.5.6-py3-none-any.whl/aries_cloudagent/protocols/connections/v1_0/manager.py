...
class ConnectionManager:
    ...
    async def create_invitation(
        ...
        igrantio_org_id = self.context.settings.get("igrantio_org_id")
        igrantio_api_key = self.context.settings.get("igrantio_api_key")
        igrantio_baseurl = self.context.settings.get("igrantio_endpoint")

        if igrantio_org_id and igrantio_api_key and igrantio_baseurl:
            organization_detail_endpoint = "/v1/organizations/{organizationID}".format(
                organizationID=igrantio_org_id)

            headers = {'Authorization': 'ApiKey {}'.format(igrantio_api_key)}

            async with aiohttp.ClientSession(headers=headers) as session:
                async with session.get(igrantio_baseurl + organization_detail_endpoint) as resp:
                    if resp.status == 200:
    ...
    async def create_did_document(
        ...
        # Mediator allocates a routing key for mobile agent to use in DIDDoc when establishing connection with another agent
        if self.context.settings.get("mobile_agent_mediator_mode"):
            wallet: BaseWallet = await self.context.inject(BaseWallet)
            routing_key_did_info = await wallet.create_local_did()

            routing_key = PublicKey(
                routing_key_did_info.did,
                f"routing-key",
                routing_key_did_info.verkey,
                PublicKeyType.ED25519_SIG_2018,
                routing_key_did_info.did,
                True,
            )
    ...
    async def fetch_connection_targets(
        ...
        if not connection.my_did:
            if (
                connection.state
                in (ConnectionRecord.STATE_INVITATION)
                and connection.initiator == ConnectionRecord.INITIATOR_EXTERNAL
            ):
                # TODO : Remember to remove this
                # Create new DID for connection
                print("\n\nCreating new DID and attach it to the connection")
                my_info = await wallet.create_local_did()
                connection.my_did = my_info.did

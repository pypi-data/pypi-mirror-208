...
from ...basic_routing.v1_0.manager import INBOX_INFO_RECORD_TYPE, ROUTE_INFO_RECORD_TYPE
...
class RoutingManager:
    ...
    async def save_forward_message_to_inbox(self, to: str, msg: bytes) -> bool:
        """Saves forward message to the corresponding inbox if available."""

        try:

            storage = await self.context.inject(BaseStorage)

            # Fetch route info record corresponding to the routedestination
            ...
            print("\n\n\n")
            print(f"Found the corresponding route destination record for {to}")
            print("\n\n\n")

            connection_id = route_info_records[0].tags.get("connection_id")

            # Fetch the corresponding inbox record
            inbox_creds = {}
            ...
            print("\n\n\n")
            print(
                f"Found the corresponding inbox info record -> inbox_name {inbox_creds['name']} and inbox_key {inbox_creds['key']}")
            print("\n\n\n")
            ...
            record = StorageRecord(
                "inbox-message", json.dumps(msg.decode("utf-8")), inbox_message_tags
            )

            await inbox_storage.add_record(record)

            await inbox.close()

            print("\n\n\n")
            print(f"Successfully saved forward message to corresponding inbox")
            print("\n\n\n")

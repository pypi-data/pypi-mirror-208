...
class ForwardHandler(BaseHandler):
    ...
    async def handle(self, context: RequestContext, responder: BaseResponder):
        ...
        if context.settings.get("mobile_agent_mediator_mode"):
            # Save message to appropriate inbox based on the 'to' field
            ...
            forward_message_save_status = await rt_mgr.save_forward_message_to_inbox(target, packed)
            if not forward_message_save_status:
                print("\n\n\n")
                print("Unable to save forward message to corresponding inbox")
                print("\n\n\n")

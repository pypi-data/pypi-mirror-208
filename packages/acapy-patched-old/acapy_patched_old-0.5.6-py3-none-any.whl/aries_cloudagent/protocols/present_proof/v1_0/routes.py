...
class V10PresentationExchangeListQueryStringSchema(OpenAPISchema):
    ...
    qr_id = fields.UUID(
        description="Data exchange QR ID",
        required=False,
        example=UUIDFour.EXAMPLE,  # typically but not necessarily a UUID4
    )

    auto_data_ex_id = fields.UUID(
        description="Data exchange template ID",
        required=False,
        example=UUIDFour.EXAMPLE,  # typically but not necessarily a UUID4
    )
...
class PresentationRequestBoundTemplateMatchInfoSchema(OpenAPISchema):
    """Path parameters and validators for sending a proof request based on data exchange template on a connection."""
    ...
    auto_data_ex_id = fields.Str(
        description="Automated dataexchange template ID", required=True, example=UUIDFour.EXAMPLE
    )
...
async def presentation_exchange_list(request: web.BaseRequest):
    ...
    post_filter = {
        k: request.query[k]
        for k in ("connection_id", "role", "state", "qr_id", "auto_data_ex_id")
...
@docs(
    tags=["present-proof"],
    summary="Sends a free presentation request bound to data exchange template",
)
@match_info_schema(PresentationRequestBoundTemplateMatchInfoSchema())
@response_schema(V10PresentationExchangeSchema(), 200)
async def presentation_exchange_send_dataexchange_template_bound_request(request: web.BaseRequest):
    """
    Request handler for sending a presentation request bound to data exchange template.
    ...
    # Retrieve dataexchange template record

    igrantio_operator_mgr = IGrantIOOperatorManager(context=context)
    auto_data_ex_record = await igrantio_operator_mgr.get_automated_data_exchange_record(auto_data_ex_id=auto_data_ex_id)
    qr_result = await igrantio_operator_mgr.create_qr_automated_data_exchange_record(auto_data_ex_id=auto_data_ex_id)
    if not qr_result:
        raise web.HTTPInternalServerError(
            reason="Failed to create QR automated data exchange record")

    if not auto_data_ex_record:
        raise web.HTTPNotFound(
            reason=f"Dataexchange template {auto_data_ex_id} not found")

    comment = auto_data_ex_record.get("presentation_request").get("name", "Proof request")
    indy_proof_request = auto_data_ex_record.get("presentation_request")
    if not indy_proof_request.get("nonce"):
        indy_proof_request["nonce"] = await generate_pr_nonce()
    ...
    pres_ex_record = None
    try:
        (pres_ex_record) = await presentation_manager.create_exchange_for_request_bound_template(
            connection_id=connection_id,
            presentation_request_message=presentation_request_message,
...
async def register(app: web.Application):
    ...
    app.add_routes(
        [
            ...
            web.post(
                "/present-proof/data-exchange-template/{auto_data_ex_id}/connection/{conn_id}/send-request",
                presentation_exchange_send_dataexchange_template_bound_request,
            ),
            web.post(

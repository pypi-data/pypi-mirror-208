...
from ...igrantio_operator.v1_0.manager import IGrantIOOperatorManager
...
class PresentationManager:
    ...
    async def receive_proposal(self):
        ...
        auto_data_ex_id = None
        qr_id = presentation_proposal_message.qr_id
        ...
        if qr_id:
            ...
            if qr_data_ex_records:
                auto_data_ex_id = qr_data_ex_records[0].tags.get("auto_data_ex_id")
            else:
                # Handle given QR ID is invalid.
                qr_id = None
    ...
    async def create_bound_request(
        ...
        """
        data_exchange_template_name = None
        data_exchange_template_version = None
        data_exchange_template_nonce = None

        # Fetch automated dataexchange template record
        if presentation_exchange_record.auto_data_ex_id:
            storage = await self.context.inject(BaseStorage)
            data_ex_records = await storage.search_records(
                type_filter=IGrantIOOperatorManager.ORG_AUTO_DATA_EXCHANGE_RECORD_TYPE,
                tag_query={
                    "auto_data_ex_id": presentation_exchange_record.auto_data_ex_id
                }
            ).fetch_all()

            if data_ex_records:
                data_exchange_template_value = json.loads(data_ex_records[0].value)
                data_exchange_template_name = data_exchange_template_value.get(
                    "name", None)
                data_exchange_template_version = data_exchange_template_value.get(
                    "version", None)
                data_exchange_template_nonce = data_exchange_template_value.get(
                    "nonce", None)

        # Create proof request based on proposal
        indy_proof_request = await (
            ...
            name=data_exchange_template_name if data_exchange_template_name else name,
            version=data_exchange_template_version if data_exchange_template_version else version,
            nonce=data_exchange_template_nonce if data_exchange_template_nonce else nonce,
            ledger=await self.context.inject(BaseLedger),
        )
        presentation_request_message = PresentationRequest(
            comment=data_exchange_template_name if data_exchange_template_name else comment,
    ...
    async def create_exchange_for_request_bound_template(
        self, connection_id: str, presentation_request_message: PresentationRequest,
        auto_data_ex_id: str = None
    ):
        """
        Create a presentation exchange record for input presentation request bound to dataexchange template

        Args:
            connection_id: connection identifier
            presentation_request_message: presentation request to use in creating
                exchange record, extracting indy proof request and thread id
            auto_data_ex_id: data exchange template record ID
        ...
        await presentation_exchange_record.save(
            self.context, reason="create (dataexchange template bound) presentation request"
        )
    ...
    async def delete_previous_presentation_records(self):
        storage = await self.context.inject(BaseStorage)
        presentation_exchange_records = await V10PresentationExchange.query(self.context, tag_filter={}, post_filter={
            "connection_id": self.context.connection_record.connection_id
        })

        if presentation_exchange_records:
            for presentation_exchange_record in presentation_exchange_records:
                await storage.delete_record(presentation_exchange_record)

...
handler = logging.FileHandler('log_file/risk_parity_budget_log_{0}.txt'.format(datetime.today().strftime('%Y%m%d')))
        ...
        if period_data['cov_df'].empty:
            ...
            try:
                if self.method == 'risk_parity':
                    ...
                    for p in [date, self.method, self.asset_type, self.asset_list, self.cov, self.lb_list, self.ub_list, self.asset_category_df, self.category_lb_list, self.category_ub_list, self.total_weight]:
                        logger.debug(p)
                    ...
                    self.cov = period_data['cov_df']
                    logger.debug('OPT Parameters In')
                    for p in [date, self.method, self.asset_type, self.asset_list, self.cov, self.lb_list, self.ub_list, self.asset_category_df, self.category_lb_list, self.category_ub_list, self.total_weight, self.risk_budget_list]:
                        logger.debug(p)

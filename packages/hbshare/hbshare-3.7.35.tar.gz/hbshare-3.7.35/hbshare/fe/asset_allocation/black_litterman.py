...
handler = logging.FileHandler('log_file/black_litterman_log_{0}.txt'.format(datetime.today().strftime('%Y%m%d')))
...
class BlackLitterman:
    def __init__(self, asset_type, asset_list, asset_type_dict, method, start_date, end_date, is_reblance, reblance_type, n, frequency, compute_risk_days,
                       ...
                       w_eq=None, risk_aversion=None, tau=None, P=None, Q=None, omega=None, processes=16):
        ...
        self.tau = 0.01 if tau is None else tau
        self.P = np.matrix(np.zeros([len(self.asset_list), len(self.asset_list)])) if P is None or P == [] else np.matrix(P)
        self.Q = np.matrix(np.zeros([len(self.asset_list)])).T if Q is None or Q == [] else np.matrix(Q).T
        self.omega = np.matrix(np.identity(len(self.asset_list))) if omega is None or omega == [] else np.matrix(omega)
    ...
    def _load(self):
        ...
        logger.debug('data_start_date')
        logger.debug(self.start_date_backup)
        logger.debug('data_end_date')
        logger.debug(self.end_date)
        logger.debug('nav_df')
        logger.debug(self.nav_df.to_json())
    ...
    def _init_data(self):
        ...
        if not (self.nav_df.empty or self.nav_df.reindex(date_list).interpolate().dropna(how='all').empty or self.nav_df.dropna(axis=1, how='all').shape[1] != len(self.asset_list)):
            ...
            for date in reblance_list:
                ...
                if min(density) < 0.3:
                    ...
                    interval_data['cov_df'] = la.inv(self.tau * cov_df_prior) + self.P.T * la.inv(self.omega) * self.P
                    interval_data['cov_df'] = correct_cov(interval_data['cov_df'])
                    interval_data['cov_df'] = la.inv(interval_data['cov_df'])
                    ...
                    interval_data['ret_df'] = interval_data['cov_df'] * (la.inv(self.tau * cov_df_prior) * ret_df_prior + self.P.T * la.inv(self.omega) * self.Q)
        ...
        logger.debug(self.preprocessed_data)
        return
    ...
    def get_optimal_one_process(self, date, period_data):
        if period_data['ret_df'].empty or period_data['cov_df'].empty:
            ...
            try:
                ...
                logger.debug('OPT Parameters In')
                for p in [date, self.method, self.asset_type, self.asset_list, self.ret, self.cov, self.risk_aversion, self.lb_list, self.ub_list, self.asset_category_df, self.category_lb_list, self.category_ub_list, self.total_weight]:
                     logger.debug(p)
            ...
            try:
                ...
                logger.debug('EF Parameters In')
                for p in [date, self.method, self.asset_type, self.asset_list, self.ret_ori, self.cov_ori, self.lb_list, self.ub_list, self.asset_category_df, self.total_weight, weight]:
                     logger.debug(p)
                ef_weight = EF(self.asset_type, self.asset_list, self.ret_ori, self.cov_ori, self.lb_list, self.ub_list, self.asset_category_df, self.total_weight, weight).solve()
                ef_weight['date'] = date
        ...
        logger.debug(date)
        logger.debug('weight')
        logger.debug(weight.to_json())
        logger.debug('status')
        logger.debug(status.to_json())
        logger.debug('ef_weight')
        logger.debug(ef_weight.to_json())
...
if __name__ == '__main__':
    ...
    weight_df, status_df, ef_weight_df = BlackLitterman(asset_type='index',  # index, fund
                                                        asset_list=['000300', '000906', 'CBA00301'],
                                                        asset_type_dict={'000300': 'MUTUAL_INDEX', '000906': 'MUTUAL_INDEX', 'CBA00301': 'MUTUAL_INDEX'},  # MUTUAL_INDEX, PRIVATE_INDEX, MARKET_INDEX, MUTUAL_FUND, PRIVATE_FUND

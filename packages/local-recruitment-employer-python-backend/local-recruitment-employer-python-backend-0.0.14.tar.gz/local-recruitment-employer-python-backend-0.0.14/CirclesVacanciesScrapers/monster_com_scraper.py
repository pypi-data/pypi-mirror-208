...
import pandas as pd
import os
from database_vacancies import DatabaseVacancies
...
class MonsterComScraper:
    ...
    def get_page_records(self, job_list):
        ...
        data = {'Job_Title': title[0], 'Company': cmpny[0], 'Location': loc[0], 'Job_Description': job_description}
    ...
    def page_scraping(self, num_of_jobs_required):
        ...
        try:
            ...
            jobs = self.driver.find_elements(By.XPATH, '//div[@class = "job-cardstyle__JobCardHeader-sc-1mbmxes-7 gUotEa"]')
            for job in jobs:
            ...
            database.add_data_to_mysql_database(df)
        except:

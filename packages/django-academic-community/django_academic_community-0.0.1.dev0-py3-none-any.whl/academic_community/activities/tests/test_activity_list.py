...
from academic_community.activities.views import ActivityList
...
@pytest.mark.parametrize(
    ...
    [
        ...
        ["members", {"members": "test"}, None],
        ["organizations", {"organizations": "test"}, None],
        ["category", {"category": 0}, lazy_fixture("category")],
    ],
)
def test_render_authenticated(
    ...
    if urlparams:
        ...
        query = ""
    try:
        response = authenticated_client.get("/activities/" + query)
    ...
    if urlparams:
        request = rf.get("/activities/" + query)
...
def test_render_anonymous(
    ...
    if urlparams:
        ...
        query = ""
    try:
        response = client.get("/activities/" + query)
    except (OperationalError, NotSupportedError):
        pytest.skip("sqlite3 does not support search")
    assert response.status_code == 200

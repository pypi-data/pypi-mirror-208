...
from academic_community.members.views import CommunityMemberList
...
@pytest.mark.parametrize(
    ...
    [
        ...
        ["last_name__istartswith", {"last_name__istartswith": "T"}, None],
        ["description__search", {"description__search": "test"}, None],
        [
            "start_date__range",
            {
        ...
        ["membership", {"membership": "test"}, None],
        ["orcid__icontains", {"orcid__icontains": "000"}, None],
        ["website__icontains", {"website__icontains": "http"}, None],
        [
            "email__email__icontains",
            {"email__email__icontains": "hereon"},
            None,
        ],
        ["phone_number__search", {"phone_number__search": "012"}, None],
        [
            "non_member_details__search",
            {"non_member_details__search": "test"},
            None,
        ],
        ["is_member", {"is_member": True}, None],
    ],
)
def test_render_authenticated(
    ...
    if urlparams:
        ...
        query = ""
    try:
        response = authenticated_client.get("/members/" + query)
    ...
    if urlparams:
        request = rf.get("/members/" + query)
        request.user = member.user  # type: ignore

        view = CommunityMemberList()
        view.setup(request)
        view.get(request)
...
def test_render_anonymous(
    ...
    if urlparams:
        ...
        query = ""
    try:
        response = client.get("/members/" + query)
    except (OperationalError, NotSupportedError):
        pytest.skip("sqlite3 does not support search")
    assert response.status_code == 200

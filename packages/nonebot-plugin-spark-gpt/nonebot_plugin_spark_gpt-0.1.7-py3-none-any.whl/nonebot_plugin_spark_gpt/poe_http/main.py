...
async def __poe_create___(
    ...
    async with creat_lock:
        try:
            ...
            is_created = await asyncio.wait_for(task,20)
        except Exception as e:
            current_userdata.is_waiting = False
            waitmsg = await matcher.send(reply_out(event, "出错了:{e}\n正在尝试重启poe client来解决"))
            task = asyncio.create_task(run_re_client())
            result = await asyncio.wait_for(task,20)
            if result :
                await matcher.finish(reply_out(event, "重启成功，请再次询问试试"))
            else:
                await matcher.finish(reply_out(event, "重启失败，请联系机器人管理员或者重新询问试试"))
        current_userdata.is_waiting = False
        if is_created:
            # # 将更新后的字典写回到JSON文件中
            botinfo = BotInfo(
    ...
    if mode == "private":
        ...
        if text in ["清除对话", "清空对话", "清除历史", "清空历史", "清空历史对话", "刷新对话"]:
            ...
            try:
                ...
                is_cleared = await asyncio.wait_for(task,20)
            except Exception as e:
                current_userdata.is_waiting = False
                waitmsg = await matcher.send(reply_out(event, "出错了:{e}\n正在尝试重启poe client来解决"))
                task = asyncio.create_task(run_re_client())
        ...
        try:
            ...
            try:
                ...
                pass
            current_userdata.is_waiting = False
            waitmsg = await matcher.send(reply_out(event, f"出错了:{e}\n正在尝试重启poe client来解决"))
            task = asyncio.create_task(run_re_client())
            result = await asyncio.wait_for(task,20)
            await bot.delete_msg(wait_msg["message_id"])
            if result :

...
from flexrunner.config import global_cf
from flexrunner.utils import util
from flexrunner.core.runners import exceptions
from flexrunner.core.runners.exceptions import ValidationFailure, StatusCodeValidationFailure, \
    ExtractFailure, ParamsError
from flexrunner.core.runners.api.client import HttpSession
from flexrunner.core.runners.api.ext.uploader import prepare_upload_step
from flexrunner.core.runners.api.builtin_loader import load_project_meta
from flexrunner.core.runners.api.parser import build_url, parse_data, parse_variables_mapping, call_func
from flexrunner.core.runners.api.response import ResponseObject
from flexrunner.core.runners.api.step import Config, ApiStep as Step
from flexrunner.core.runners.api.utils import merge_variables
from flexrunner.base.models import TestTime
...
class ApiRunner(object):
    ...
    def with_project_meta(self, project_meta: ProjectMeta) -> "FlexRunner":
        self.__project_meta = project_meta
        return self

    def with_session(self, session: HttpSession) -> "FlexRunner":
        self.__session = session
        return self

    def with_case_id(self, case_id: Text) -> "FlexRunner":
        self.__case_id = case_id
        return self

    def with_variables(self, variables: VariablesMapping) -> "FlexRunner":
        self.__session_variables = variables
        return self

    def with_export(self, export: List[Text]) -> "FlexRunner":
    ...
    def run_testcase(self, testcase: TCase) -> "FlexRunner":
        """run specified testcase

        Examples:
            >>> testcase_obj = TCase(config=TConfig(...), teststeps=[TStep(...)])
            >>> FlexRunner().with_project_meta(project_meta).run_testcase(testcase_obj)
    ...
    def test_start(self, param: Dict = None) -> "FlexRunner":
        """主入口，pytest执行"""
        try:

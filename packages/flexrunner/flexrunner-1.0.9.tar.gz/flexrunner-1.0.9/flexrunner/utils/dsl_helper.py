...
import flexrunner.core.global_resource as gr
...
def params_to_dic(dsl_params, def_key="selector"):
    ...
    logger.info("result_dic: {}".format(result_dic))
    return result_dic
...
def is_number(s):
    ...
    try:
        ...
        logger.error(f"param {s} is not a number!")
    try:
        import unicodedata
        unicodedata.numeric(s)
        return True
    except (TypeError, ValueError):
        logger.error(f"param {s} cannot turn to a number!")
    return False
...
def get_use_define_param(context, param_name):
    ...
    logger.info(f'use_define: {use_define}')
    params = [i for i in use_define if param_name + '=' in i]
    user_data = {}
    if len(params) > 0:
        if len(params) > 1:
            logger.error(f'Cannot customize multiple parameters with the same '
                      f'name:{params}')
        ...
        for (k, v) in kwargs.items():
            if v is None:
                if hasattr(context, k):
                    ...
                    logger.warning(f'[ele_wrap] step param:[{k}] is none.')
                    continue
            v = replace_str(v)
            if 'selector' in k:
                selector_str = v

...
def setup_arg_parsing():
    ...
    parser_addpropasmetriclabel.add_argument('--search', required=False, help='Search String to filter resources as it would be entered under Resources -> Search')
    parser_addpropasmetriclabel.add_argument('--filter', required=False, help='Post-query filter on resources.  Python expression that will evaluate to True or False such as alert["resource"]["name"].startswith("prod")')
    parser_addpropasmetriclabel.add_argument('--property', required=True, help='Resource property in dot notation to be with values to be added as a custattr values, for example attributes.accountName (Must be in content returned by getresources)')

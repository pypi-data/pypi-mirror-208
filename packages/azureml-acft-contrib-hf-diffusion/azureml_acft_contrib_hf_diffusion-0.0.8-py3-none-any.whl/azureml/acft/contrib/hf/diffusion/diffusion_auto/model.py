...
class AzuremlAutoEncoderKL(AutoencoderKL):
    ...
    def from_pretrained(cls, hf_model_name_or_path: str, **kwargs) -> AutoencoderKL:
        ...
        vae = super(cls, cls).from_pretrained(hf_model_name_or_path, subfolder="vae", revision=revision)
        return AzuremlAutoEncoderKL._post_init(vae, freeze_weights=freeze_weights, device=device, weight_dtype=weight_dtype)
...
class AzuremlUNet2DConditionModel(UNet2DConditionModel):
    ...
    def from_pretrained(cls, hf_model_name_or_path: str, **kwargs) -> UNet2DConditionModel:
        ...
        unet = super(cls, cls).from_pretrained(hf_model_name_or_path, subfolder="unet", revision=revision)
...
class AzuremlCLIPTextEncoder(CLIPTextModel):
    ...
    def from_pretrained(cls, hf_model_name_or_path: str, **kwargs) -> CLIPTextModel:
        ...
        device: int = kwargs.get("device", torch.device("cuda"))
        weight_dtype: torch.dtype = kwargs.get("weight_dtype", torch.float32)
        text_encoder = super(cls, cls).from_pretrained(hf_model_name_or_path, subfolder="text_encoder", revision=revision)

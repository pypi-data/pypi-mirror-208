...
class BaseRecLabelDecode(object):
    ...
    def decode(self, text_index, text_prob=None, is_remove_duplicate=False):
        ...
        for batch_idx in range(batch_size):
            ...
            t = text_index[batch_idx]
            cut_index = 0
            for i in range(length):
                if t[i] != 0 and (not (i > 0 and t[i - 1] == t[i])):
                    if i == length - 1:
                        pos.append([i - int((i - cut_index) / 2), i + 1])
                    elif t[i+1] == t[i]:
                        pos.append([i - int((i - cut_index) / 2), i + 3])
                    else:
                        pos.append([i - int((i - cut_index) / 2), i + 2])
                    cut_index = i
            if len(pos) > 0:
                pos = (np.array(pos)-np.min(pos))/(np.max(pos)-np.min(pos))
            ...
            result_list.append((text, np.mean(conf_list).tolist(), pos))
        return result_list

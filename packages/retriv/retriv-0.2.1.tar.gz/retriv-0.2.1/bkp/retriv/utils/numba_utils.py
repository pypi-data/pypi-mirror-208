...
@njit(cache=True)
def arg_unsorted_top_k(array, k):
    sorted_indices = np.zeros(k, dtype=np.int32)
    min_value = 0.0
    min_value_idx = 0

    for value in array:
        if value > min_value:
            sorted_indices[min_value_idx] = value
            min_value_idx = sorted_indices.argmin()
            min_value = sorted_indices[min_value_idx]

    return (array >= min_value).nonzero()[0][::-1][:k]

...
def main():
    ...
    if args.function == 'pinyin':
        ...
        # 这里的输入路径（args.input_datasets[0].get('input_dir')），定义的字典键很重要，关系到程序整体框架设计
        dataset_info = data_info.get_dir_currency(args.input_datasets[0].get('input_dir'))  # 单进程封装数据处理
        print('异步耗时')
        ...
        if args.function == 'merge':  # 合并功能
            if args.rectangle_merge:  # 删除人工标注矩形框，然后与模型预测出来的矩形框进行合并功能
                # 加载模型预测的labelme数据集
                model_dataset_info = data_info.get_dir_currency(args.input_datasets[0].get('model_input_dir'))
                # 先删除人工标注的矩形框

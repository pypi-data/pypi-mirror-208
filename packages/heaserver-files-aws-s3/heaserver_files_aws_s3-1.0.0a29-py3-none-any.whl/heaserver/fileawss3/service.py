...
async def put_file(request: web.Request) -> web.Response:
    ...
    try:
        if key != file.key:
            s3_client.copy_object(Bucket=bucket_id, CopySource=f'{bucket_id}/{key}', Key=file.key)
            s3_client.delete_object(Bucket=bucket_id, Key=key)

        s3_client.delete_object_tagging(Bucket=bucket_id, Key=file.key)
        s3_client.put_object_tagging(Bucket=bucket_id, Key=file.key, Tagging={'TagSet': await _to_aws_tags(file.tags)})

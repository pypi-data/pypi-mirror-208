...
def build_pool_from_file(data_dir: str, plan_start_date: str):
    """
    ...
    df_job = pd.read_csv(os.path.join(data_dir, "order.csv"))
    return _build(
        df_machine,
        df_product,
        df_process,
...
def _build(
    ...
    for index, row in df_job.iterrows():
        pool.job_dict[row["order_id"]] = JobClass(
            job_id=row["order_id"],
        ...
        process_ops = copy.deepcopy(pool.product_dict[row["product_id"]].operations)
        for op in process_ops.values():
            op_id = "{}-{}".format(row["order_id"], op.id)
            df_machine_op = df_product_process_machine[
                (df_product_process_machine["product_id"] == row["product_id"])
                & (df_product_process_machine["process_id"] == int(op.process_id))
            ]
            machine_times = dict(zip(df_machine_op["machine_id"], row["qty"] * df_machine_op["process_time(s)"]))
            op.id = op_id
            op.demand = row["qty"]
            op.job_id = row["order_id"]
            op.process_id = int(op.process_id)
            op.pre_ops = ["{}-{}".format(row["order_id"], x) for x in op.pre_ops]
            op.next_ops = ["{}-{}".format(row["order_id"], x) for x in op.next_ops]
            ...
            pool.job_dict[row["order_id"]].operations.append(op_id)
            if len(op.pre_ops) == 0:
                pool.job_dict[row["order_id"]].start_operations.append(op_id)
...
if __name__ == "__main__":
    ...
    pool = build_pool_from_file("D:\shangjian\sj_tool\data",plan_start_date)

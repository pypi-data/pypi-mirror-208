...
class CommandType:
    ...
    char_base_fmt: pt.FT = pt.NOOP_STYLE
    description: str = None
    is_default: bool = False

    _values: ClassVar[set["CommandType"]] = set()
    ...
    def __hash__(self) -> int:
        return hash(f'{self.char or ""}{self.name}{self.fmt}{self.char_base_fmt}')

    def get_char_fmt(self) -> pt.Style:
        return FrozenStyle(pt.make_style(self.char_base_fmt or self.fmt), bold=True)

    def format_char(self) -> pt.Text:
        return (
            pt.Fragment("[")
            + pt.Fragment(self.char, self.get_char_fmt())
            + pt.Fragment(f"] {self.name}")
        )

    def format_as_own(self) -> pt.Fragment:
        own_st = FrozenStyle(
            pt.make_style(self.get_char_fmt()),
            bg=0xFFFFFF,
            dim=True,
            bold=True,
            inversed=True,
        )
        return pt.Fragment(f'{self.char or " ":^3s}', own_st)
...
CMDTYPE_BUILTIN = CommandType(
    ...
    sorter=15,
    fmt=pt.cv.BLUE,
    description="This is a %3s|builtin|es7s/core component written in Python 3 (G2/G3).",
    is_default=True,
)
...
CMDTYPE_EXTERNAL = CommandType(
    ...
    "es7s/core, but is (usually) installed as a part of es7s system. Can "
...
CMDTYPE_X11 = CommandType(
    ...
    sorter=30,
    fmt=pt.cv.MAGENTA,
    description="This is a builtin component that works in %3s|X11 environment|only.",
)
CMDTYPE_DYNAMIC = CommandType(
    ...
    sorter=40,
    fmt=pt.cv.GREEN,
    description="This is a builtin component that %3s|adjusts|the output depending on terminal size.",
)

...
class HelpFormatter(click.HelpFormatter):
    ...
    class CTIReplacer(pt.StringReplacer):
        COMMAND_TYPE_NAME_MAP: t.Dict[str, CommandType] = {
            ct.name: ct for ct in CommandType._values
        }
        ...
        def replace(self, m: t.Match) -> str:
            if ct := self.COMMAND_TYPE_NAME_MAP.get(m.group(1), None):
                return get_stdout().render(ct.format_char())
            return m.group()
...
class CliBaseCommand(click.Command):
    ...
    def format_own_command_type(self, ctx: Context, formatter: HelpFormatter):
        ...
        with formatter.indentation():
            ct_icon = get_stdout().render(ct.format_as_own())
...
class CliGroup(click.Group, CliBaseCommand):
    ...
    def format_subcommand_types(self, formatter: HelpFormatter):
        cts = [*filter(lambda ct: ct.char, {*self.get_command_types()})]  # and not ct.is_default
        if not len(cts):
            return

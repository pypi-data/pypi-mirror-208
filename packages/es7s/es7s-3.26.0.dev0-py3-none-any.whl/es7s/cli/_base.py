    ...
    def format_own_type(self, ctx: Context, formatter: HelpFormatter):
        with formatter.indentation():
            ...
            for ct in [self.get_command_type(), self.get_command_trait()]:
                ct_icon = get_stdout().render(formatter.format_command_own_type(ct))
                ct_description = (ct.description % ct_icon if "%s" in ct.description else ct.description)

                if ct_description.count("|") == 2:
                    left, hightlight, right = ct_description.split("|", 2)
                    descriptions.append((
                        left
                        + get_stdout().render(" " + hightlight + " ", ct.get_own_label_fmt())
                        + right
                    ))
                else:

...
_print_colors_4bit() {
    ...
    local -a color_y_values_ext=("7;1" "7;2")
    local -a color_preset_prefix=""
    local -a color_presets_before=("" 3 4 5 8 9 21 39 53)
    ...
    if [[ -n "$MODE_BACKGROUND" ]] ; then
        ...
        color_y_values=("" 1 2 30 7)
        color_preset_prefix="30;47;"
        color_presets_before=(3 4 5 8 9 21 49 53)
    ...
    for color_extra in "${color_y_values[@]}" ; do
        printf "${_d}[${_f}%4s${_d}]${_f}" "${color_extra}"
    done
    printn
    ...
    for color_main in "${colors[@]}" ; do
        [[ "$color_main" == $"\n" ]] && printn && continue
        printf "%s${_d}[] ${_f}" "$(printf "%$(( margin + 3 ))s"  "${color_main}" | sed -Ee "s/${color_preset_prefix:-"^$"}/$_d&$_f/")"
        for color_extra in "${color_y_values[@]}" ; do
            local color="${color_main}${color_extra:+";$color_extra"}"
            printf "%s es7s %s" "$(_cs "${color}")" "$_f"
        done
        local description_key="${color_main#$color_preset_prefix}"

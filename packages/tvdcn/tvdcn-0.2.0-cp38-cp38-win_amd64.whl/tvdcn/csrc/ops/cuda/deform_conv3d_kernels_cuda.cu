    ...
    namespace ops {
        namespace {
            ...
            __device__ __forceinline__ scalar_t sample(
                    const at::GenericPackedTensorAccessor<scalar_t, 5> input,
                    const int b,
                    const int c,
                    const int depth,
                    const int height,
            ...
            __device__ __forceinline__ void insert(
                    at::GenericPackedTensorAccessor<scalar_t, 5> output,
                    const int b,
                    const int c,
                    const int depth,
                    const int height,
        ...
        static __global__ void vol2col_kernel(
                const int n_kernels,
                const at::GenericPackedTensorAccessor<scalar_t, 5> input,
                const at::GenericPackedTensorAccessor<scalar_t, 9> offset,
                const at::GenericPackedTensorAccessor<scalar_t, 8> mask,
                const int depth,
                const int height,
                const int width,
                const int weight_d,
                ...
                at::GenericPackedTensorAccessor<scalar_t, 8> columns) {
        ...
        void vol2col_cuda(
            ...
            AT_DISPATCH_FLOATING_TYPES_AND_HALF(
                ...
                auto columns_accessor = columns.generic_packed_accessor<scalar_t, 8>();
                TVDCN_DISPATCH_CONDITION2(deformable, modulated, ([&] {
                    vol2col_kernel<deformable, modulated><<<blocks, threads>>>(
                            n_kernels,
                            input.generic_packed_accessor<scalar_t, 5>(),
                            offset.generic_packed_accessor<scalar_t, 9>(),
                            mask.generic_packed_accessor<scalar_t, 8>(),
                            depth,
                            height,
                            width,
                            weight_d,
        ...
        static __global__ void col2vol_kernel(
                const int n_kernels,
                const at::GenericPackedTensorAccessor<scalar_t, 8> columns,
                const at::GenericPackedTensorAccessor<scalar_t, 9> offset,
                const at::GenericPackedTensorAccessor<scalar_t, 8> mask,
                const int in_channels,
                const int depth,
                ...
                const int c_per_offset_group,
                const int c_per_mask_group,
                at::GenericPackedTensorAccessor<scalar_t, 5> grad_input) {
            CUDA_1D_KERNEL_LOOP(index, n_kernels) {
        ...
        void col2vol_cuda(
            ...
            AT_DISPATCH_FLOATING_TYPES_AND_HALF(
                ...
                auto grad_input_accessor = grad_input.generic_packed_accessor<scalar_t, 5>();
                TVDCN_DISPATCH_CONDITION2(deformable, modulated, ([&] {
                    col2vol_kernel<deformable, modulated><<<blocks, threads>>>(
                            n_kernels,
                            columns.generic_packed_accessor<scalar_t, 8>(),
                            offset.generic_packed_accessor<scalar_t, 9>(),
                            mask.generic_packed_accessor<scalar_t, 8>(),
                            in_channels,
                            depth,
        ...
        static __global__ void deform_conv3d_compute_grad_offset_kernel(
                ...
                const int c_per_offset_group,
                const int c_per_mask_group,
                at::GenericPackedTensorAccessor<scalar_t, 9> grad_offset) {
            CUDA_1D_KERNEL_LOOP(index, n_kernels) {
                const int o = index % 3;
        ...
        void deform_conv3d_compute_grad_offset_cuda(
            ...
            AT_DISPATCH_FLOATING_TYPES_AND_HALF(
                ...
                auto grad_offset_accessor = grad_offset.generic_packed_accessor<scalar_t, 9>();
        ...
        static __global__ void deform_conv3d_compute_grad_mask_kernel(
                ...
                const int c_per_offset_group,
                const int c_per_mask_group,
                at::GenericPackedTensorAccessor<scalar_t, 8> grad_mask) {
            CUDA_1D_KERNEL_LOOP(index, n_kernels) {
                const int k = index % weight_w;
                const int j = (index / weight_w) % weight_h;
                const int i = (index / (weight_w * weight_h)) % weight_d;
        ...
        void deform_conv3d_compute_grad_mask_cuda(
            ...
            AT_DISPATCH_FLOATING_TYPES_AND_HALF(
                ...
                auto grad_mask_accessor = grad_mask.generic_packed_accessor<scalar_t, 8>();

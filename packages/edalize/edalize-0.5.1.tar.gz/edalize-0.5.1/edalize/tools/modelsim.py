...
class Modelsim(Edatool):
    ...
    def setup(self, edam):
        ...
        if common_compilation:
            args = vlog_options + vlog_defines
        ...
        self.commands.add(
            ["vsim", "-c"] + vsim_options + _parameters +
            ["$(EXTRA_OPTIONS)", "-do", '"run -all; quit -code [expr [coverage attribute -name TESTSTATUS -concise] >= 2 ? [coverage attribute -name TESTSTATUS -concise] : 0]; exit"', self.toplevel],
            ["run"],
            [db_file])

        self.commands.add(
            ["vsim", "-gui"] + vsim_options + _parameters + ["$(EXTRA_OPTIONS)", self.toplevel],
            ["run-gui"],
            [db_file])
    ...
    def write_config_files(self):
        ...
        for f in self.tcl_files:
            tcl_main.write(f"do {f}\n")

        f = Path(self.work_root) / "edalize_main.tcl"
        self.update_config_file(f, tcl_main)

        tcl_build_rtl = "\n".join(self.tcl_build_rtl)
        
        f = Path(self.work_root) / "edalize_build_rtl.tcl"
        self.update_config_file(f, tcl_build_rtl)

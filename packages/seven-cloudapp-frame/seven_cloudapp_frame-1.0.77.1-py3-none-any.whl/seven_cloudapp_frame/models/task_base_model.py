...
@LastEditTime: 2023-05-06 19:14:16
...
class TaskBaseModel(FrameBaseModel):
        ...
        for task_info_dict in task_info_list:
            ...
            #购买指定商品
                ...
                if task_count.complete_count >= limit_num:
                    ...
                    result["status"] = 1 if task_count.now_count >= satisfy_num else 0
                result["desc"] = f"{complete_type_name}完成上限{limit_num}次，当前剩余{limit_num-task_count.complete_count}次"
                result["btn_text"] = ["去购买", "领取", "已完成"]
                result["route_url"] = "task_appoint_goods"
                result_list.append(result)
    ...
    def process_buy_appoint_goods_task(self,app_id,act_id,module_id,user_id,login_token,handler_name,order_data=[],app_key="",app_secret="",is_log=False,check_user_nick=True,continue_request_expire=5,is_stat=True):
        """
